...
begin
  ...
  for n := 0 to psleep_c do
    psleep[cpt, n] := psleep[pt_i, n];   // spacings.

  plain_track_form.plain_track_spacings_listbox.Items.Strings[cpt] :=
    plain_track_form.plain_track_spacings_listbox.Items.Strings[pt_i];   // name.
end;
//______________________________________________________________________________________
...
const
  spacing_help_str: string = '      Custom  Rail  Lengths  and  Sleeper  Spacings'
    ...
    + '||If staggered rail joints are wanted (e.g. for US and some Irish practice), click the REAL > PLAIN TRACK OPTIONS > RAIL JOINT MARKS > STAGGERED menu item. This omits alternate joints on each rail.' + ' Then set here a "rail length" which is half of the real rail length, and enter the sleeper (tie) spacings for each half-length.' + '||If CWR (long-welded rail) is wanted, click the REAL > PLAIN TRACK OPTIONS > RAIL JOINT MARKS > NONE menu item.' + ' Then set here a dummy "rail length" which is equal to the sleeper spacing, and have a single sleeper spaced at half that distance from the dummy "joint".';

  adopt_help_str: string =
    '      Adopt  Control  Template  Settings  for  Rail  Lengths  and  Sleeper  Spacings'
    ...
    + '||N.B. Although adopting the control template settings serves no apparent purpose if the control template is already set to one of the existing settings in the list,' + ' this can be a useful way of pre-setting the custom data before modifying it.';
...
begin
  ...
  with plain_track_form do begin
    ...
    try
      if cpt > 10 then begin
        alert(6, '    control  template  custom  slot  selected',
          'The lowermost slot in the list is reserved for the plain track settings from the most recent control template which contained custom plain track data.' + '||The settings in this slot are maintained automatically by Templot, it is not possible to change them manually.' + '||You can select these custom settings for the control template if it is currently set to one of the standard plain track settings.',
          '', '', '', '', '', 'O K', 0);
        EXIT;
      end;
      ...
      if railen[cpt] <> 0 then begin
        i := alert(4, '    existing  custom  setting  ( slot  ' + str + ' )',
          ...
          '', '', '', 'clear  existing  custom  data  in  slot  ' +
          str, 'cancel  -  no  change', 'modify  existing  custom  data  in  slot  ' + str, 0);
        if i = 4 then
          railen[cpt] := 0;
        if i = 5 then
          EXIT;
      ...
      with math_form do begin
        ...
        if (ModalResult = mrOk) and (Trim(math_editbox.Text) <> '') then
          plain_track_spacings_listbox.Items.Strings[cpt] := '  ' + Trim(math_editbox.Text);
      ...
      if n <> 1 then
        run_error(210);
      if getdims('custom  plain  track  ( slot  ' + str + ' )', spacing_help_str,
        plain_track_form, n, od) = True then begin
        if od[0] = def_req then
          railen[cpt] := 720            // 60ft rails default.
        else
          railen[cpt] := ABS(od[0]);

        if od[1] = def_req then
          psleep[cpt, 0] := 12           // 12" default.
        else
          psleep[cpt, 0] := ABS(od[1]);
        ...
        repeat
          ...
          if getdims('custom  sleeper  spacings', spacing_help_str,
            plain_track_form, n, od) = True then begin
            if psleep[cpt, (i - 1)] <> 0 then begin
              psleep[cpt, i] := ABS(od[0]);
              Inc(custom_count);
            end
            else
              BREAK;
            if psleep[cpt, i] <> 0 then begin
              psleep[cpt, i + 1] := ABS(od[1]);
              Inc(custom_count);
            end
            else
              BREAK;
            if psleep[cpt, (i + 1)] <> 0 then begin
              psleep[cpt, i + 2] := ABS(od[2]);
              Inc(custom_count);
            end
            else
              BREAK;
            if psleep[cpt, (i + 2)] <> 0 then begin
              psleep[cpt, i + 3] := ABS(od[3]);
              Inc(custom_count);
            end
            else
              BREAK;
            if psleep[cpt, (i + 3)] <> 0 then begin
              psleep[cpt, i + 4] := ABS(od[4]);
              Inc(custom_count);
            end
            else
              BREAK;
            if psleep[cpt, (i + 4)] <> 0 then begin
              psleep[cpt, i + 5] := ABS(od[5]);
              Inc(custom_count);
            end
            else
              BREAK;
            if psleep[cpt, (i + 5)] <> 0 then begin
              psleep[cpt, i + 6] := ABS(od[6]);
              Inc(custom_count);
            end
            else
              BREAK;
            if psleep[cpt, (i + 6)] <> 0 then begin
              psleep[cpt, i + 7] := ABS(od[7]);
              Inc(custom_count);
            end
            else
              BREAK;
        ...
        sleeper_count[cpt] := custom_count - 1;
        // number of sleepers per length (was incremented on final zero entry).
      end
      else
        EXIT;
      Result := True;
    finally
      plain_track_spacings_listbox.ItemIndex := cpt;
      // ?? Delphi bug - gets changed when strings modified?
  ...
  repeat
    ...
    then
      ...
      if alert(4, '    new  custom  plain  track ?  ( slot  ' + str + ' )',
        ...
        '', '', '', '', 'cancel',
        'O K  -  enter  custom  plain  track  data  for  slot  ' + str, 0) = 5 then
        EXIT;
      ...
      then begin
        ...
        then
          pt_i := plain_track_spacing_size;  // change plain track setting, and show form again.
      end;
    end;
  until pt_ok = True;
...
const
  plain_track_help_str: string =
    '        `0Plain  Track  Rail  Lengths`9|        `0and  Sleeper  Spacings`9'
    + '||(For information about using plain track templates in track planning, click the `0about using plain track templates`1 button below.)' + '||Use this dialog to select the rail lengths and sleeper spacings to be used' + ' when generating plain track templates, and also for the approach and exit track on turnout templates.' + '||Select the rail length and sleeper spacings you require by clicking the list, and then click the `0OK`z button.' + '||Clicking the `0RESTORE`1 button will restore the previous setting.' + '||The following notes refer primarily to UK trackwork practice:' + '||The 30ft and 45ft lengths are generally more suitable for the pre-grouping era.' + ' Longer 60ft rails became the norm for new bullhead track after the grouping of railway companies in 1923, although of' + ' course shorter rails remained in use for many years and could be found on secondary routes well into the BR period.' + '||The number of sleepers per rail length was increased on sharp curves.' + '| For 60ft rails the rules are these:' + '|Over 40 chains radius and straight track : 24 sleepers per length.' + '|40-20 chains radius : 25 sleepers per length.' + '|under 20 chains radius : 26 sleepers per length.' + '||green_panel_begintree.gif 20 chains radius is over 5000 mm (17ft) in 4mm scale and over 9000 mm (30ft) in 7mm scale,' + ' so clearly most model curves should use the 26 sleeper spacing to be correct.' + '||In most cases, however, we are trying to disguise the sharpness of our curves, so a wider spacing will look more in keeping.' + ' 25 sleepers per length is a good compromise for most model track.green_panel_end' + '|The sleepers are not equally spaced along the rail length. Those nearer the rail joints are progressively closed up to strengthen the joints.' + '||If preferred you can enter custom rail lengths and sleeper spacings. Select one of the 6 custom slots in the list, then click the `0CUSTOM SETTINGS...`1 button and read the help notes for more information.' + '||You can swap between your custom settings and the standard settings without losing the custom dimensions.' + ' To save these custom dimensions in a template data file, ensure that there is at least one template in the storage box which uses them.' + '||For staggered rail joints (US practice) and CWR (long-welded rail), use a custom setting and read the help notes.' + '||The bottom slot in the list is reserved for the plain track settings from the most recent control template which contained custom plain track data.' + ' The settings in this slot are maintained automatically by Templot, it is not possible to change them manually.' + ' You can select these custom settings for the control template if it is currently set to one of the standard plain track settings.' + '||This dialog is concerned only with the rail lengths and sleeper spacings. To change the sleeper size, click the `0REAL > TIMBERING > TIMBERING DATA...`1 menu item.' + '||To change the position of the rail joints (plain track only) click the `0REAL > PLAIN TRACK OPTIONS > ROLL RAILS AND SLEEPERS BY...`1 menu item.';

begin
  if help(0, plain_track_help_str, 'about  using  plain  track  templates') = 1 then
    if help(0, pt_templates_help_str, 'about  plain  track  settings') = 1 then
      help_button.Click;
end;
...
const
  chat_str: string = '    Plain Track Chat' +
    '||The pre-grouping railways used a wide variety of rail lengths and sleeper spacings, and sometimes wider sleepers next to the joint,' + ' or a single joint sleeper with a combined chair/fishplate. I would like to include some of these in the list, but accurate spacing info is' + ' hard to come by.' + '||If you are using custom spacings based on prototype information, I would be very pleased to learn the details,' + ' so that I can offer a wider choice of pre-set options on this form.';
