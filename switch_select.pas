...
function get_switch_data(sw_group, sw_size: integer; var sw_info: Tswitch_info): boolean;
  forward;  // return switch data from listbox entries.
      ...
      with Tswitch(Items.Objects[ItemIndex]) do begin
        ...
        if list_switch_info.valid_data = False then begin
          if alert(6, '     no  data',
            ...
            '', '', '', '?  help', '', 'continue', 4) = 4 then
            help_button.Click;
          if list_switch_info.group_code > -1 then
            restore_settings_button.Click;    // restore unless a custom slot.
          EXIT;
        end;
      end;//with
    end;//with
...
begin
  ...
  if ((Pos('REA', listed_str) > 0) or (Pos('GWR', listed_str) > 0)) and
    ...
    i := alert(4, 'php/702    bullhead  switch  selected',
      ...
      '', '', '', 'leave  as  FB  rail  and  continue', 'cancel',
      'change  to  BH  rail  and  continue', 0);

    if i = 5 then
      EXIT;  // leave list showing to try another

    if i = 6 then
      pad_form.bullhead_rails_menu_entry.Click;
  ...
  if ((Pos('FB-109', listed_str) > 0) or (Pos('BS-110A', listed_str) > 0) or
    ...
    i := alert(4, 'php/702    flat-bottom  switch  selected',
      ...
      '', '', '', 'leave  as  BH  rail  and  continue', 'cancel',
      'change  to  FB  rail  and  continue', 0);

    if i = 5 then
      EXIT;  // leave list showing to try another

    if i = 6 then
      pad_form.flatbottom_rails_menu_entry.Click;
...
begin
  ...
  if switch_info_showing = True then
    show_info_button.Click;  // close it
end;
//______________________________________________________________________________
  ...
  with switch_selector_listbox do begin
    ...
    with Tswitch(Items.Objects[ItemIndex]) do begin
      ...
      if list_switch_info.valid_data = True then begin
        temp_str := list_switch_info.sw_name_str;
        // !!! (Delphi bug - compiler gets confused about parameter string length if not assigned to a local string.)
        ...
        if alert(7, '    existing  custom  switch  ( slot  ' + IntToStr(
          ...
          + ' You can have a different custom switch in each of the 8 slots.',
          '', '', '', '', 'cancel  -  no  change',
          'enter  new  or  modified  custom  switch  data  in  slot  ' + IntToStr(
          ABS(list_switch_info.group_code)) + '      ', 0) = 5 then
          EXIT;
      end;
      ...
      then begin
        ...
        Items.Strings[ItemIndex] :=
          '  custom  ' + IntToStr(ABS(list_switch_info.group_code)) + ' :  ' + csi.sw_name_str;

        new_switch_info.group_code := list_switch_info.group_code;
        // -1 to -8 for custom switches.
        new_switch_info.size_code := 1;
        // size within group (0=empty slot, 1=shortest). 0.77.a  7-6-02.
        new_switch_info.group_count := 1;
        // number of switches in this group (max csi.size_code in this group, min size is always 1).

        list_switch_info := new_switch_info;
        ItemIndex := item_index;
        // Delphi bug? - needed to show new string selected.

        switch_selector_listbox.ItemIndex := item_index;  // 208a
        if switch_info_showing = True then
          show_switch_info(full_size_mm_radiobutton.Checked, switch_info_showing);  // 208a
        switch_selector_listbox.SetFocus;
        EXIT;
      end;
      ...
      with math_form do begin
        ...
        if list_switch_info.valid_data = False then
          math_editbox.Text := 'my custom switch'
        else
          math_editbox.Text := list_switch_info.sw_name_str;

        Caption := '  custom  switch  name ...';
        big_label.Caption := insert_crlf_str(
          '|||||Enter below a name or reference for your custom switch.'
          +
          '||(If you cancel or leave this blank your switch will be named "untitled custom switch".)');

        do_show_modal(math_form);  // 212a   ShowModal

        if (math_editbox.Text = '') or (ModalResult <> mrOk) then
          custom_str := 'untitled custom switch'
        else
          custom_str := math_editbox.Text;
      ...
      if list_switch_info.valid_data then begin
        ...
        exists_str :=
          '||If you are intending to modify the existing custom switch in this slot, click AS EXISTING CUSTOM SWITCH.';
      end
      else begin
        temp_str := '';
        exists_str := '';
      ...
      repeat
        i := alert(4, '  data  for  ' + custom_str + ' ?',
          ...
          '||' + custom_str + exists_str, 'more  information', temp_str,
          'curved  switch', 'semi - curved  switch', 'cancel', 'straight  switch', 1);
        if i = 1 then
          alert_help(0, help_str, '');
      until i <> 1;
      ...
      if i = 3 then begin    // curved switch...
        ...
        if (list_switch_info.sw_pattern <> spStraightOrCurved)
          ...
          // set GWR B default curved switch if existing data not a curved switch.
          then
            run_error(82);                         // ?????? no GWR B switch in list?
        end
        else
        ...
        putdim(lead_help_str, 2, 'lead length to heel (incl. planing, full-size inches)',
          new_switch_info.heel_lead_inches, True, True, True, False);
        // no neg, no preset, no zero, don't terminate on zero.
        putdim(offset_help_str, 2, 'offset at the heel (full-size inches)',
          new_switch_info.heel_offset_inches, True, True, True, False);  // ditto...
        putdim(radius_help_str, 2, 'switch radius in FULL-SIZE inches',
          new_switch_info.switch_radius_inchormax, True, True, True, False);
        putdim(joint_help_str, 2, 'stock-rail joint to toe (full-size inches)',
          new_switch_info.switch_front_inches, True, True, False, False);
        // no neg, no preset, zero ok, don't terminate on zero.
        putdim(rail_help_str, 2, 'length of switch rail (blade) (full-size inches)',
          new_switch_info.switch_rail, True, True, False, False); // ditto...
        n := putdim(rail_help_str, 2, 'length of stock rail from joint (full-size inches)',
          new_switch_info.stock_rail, True, True, False, False);

        if n <> 5 then
          run_error(201);
        if getdims(data_caption_str, help_str, switch_select_form, n, od) =
          True then begin
          new_switch_info.heel_lead_inches := od[0];         // lead to heel.
          new_switch_info.heel_offset_inches := od[1];       // offset at heel.
          new_switch_info.switch_radius_inchormax := od[2];
          // switch radius (INCHES).
      ...
      if i = 4 then begin    // semi-curved switch ...
        ...
        putdim(angle_help_str, 0, 'straight planing unit angle 1:',
          new_switch_info.planing_angle, True, True, True, False);
        putdim(lead_help_str, 2, 'lead length to heel (incl. planing, full-size inches)',
          new_switch_info.heel_lead_inches, True, True, True, False);
        // no neg, no preset, no zero, don't terminate on zero.
        putdim(radius_help_str, 2, 'switch radius in FULL-SIZE inches',
        ...
        // no neg, no preset, zero ok, don't terminate on zero.
        putdim(rail_help_str, 2, 'length of switch rail (blade) (full-size inches)',
          new_switch_info.switch_rail, True, True, False, False);
        n := putdim(rail_help_str, 2, 'length of stock rail from joint (full-size inches)',
          new_switch_info.stock_rail, True, True, False, False);
        ...
        if getdims(data_caption_str, help_str, switch_select_form, n, od) =
          ...
          new_switch_info.switch_radius_inchormax := od[3];
          // switch radius (INCHES).
      ...
      if i = 6 then begin                    // straight switch...
        ...
        if (list_switch_info.sw_pattern <> spStraightOrCurved)
          ...
          // set 12ft default straight switch if existing data not a straight switch.
          then
            run_error(82);                         // ?????? no 12ft straight switch in list?
        end
        else
          new_switch_info := list_switch_info;               // use existing data as default.
        ...
        if getdims(data_caption_str, help_str, switch_select_form, n, od) =
          ...
          new_switch_info.heel_lead_inches := od[0];           // lead to heel.
          new_switch_info.heel_offset_inches := od[1];         // offset at heel.
          new_switch_info.switch_radius_inchormax := max_rad;
          // for straight switch.
          new_switch_info.switch_front_inches := od[2];        // joint to toe.
          new_switch_info.switch_rail := od[3];
          // switch rail length.
      ...
      repeat
        i := alert(4, '    joggled  stock  rails ?',
          ...
          '', '', '?  help', 'yes  -  make  joggled  stock  rails  optional',
          '', 'no  -  plain  stock  rails  only', 3);
        if i = 3 then
          alert_help(0, joggled_help_str, '');
      until i <> 3;

      case i of
        4: begin   // get joggle data.

          putdim(joggle_depth_help_str, 2, 'sideways depth of joggle (full-size inches)',
          ...
          if n <> 1 then
            run_error(229);
          if getdims(data_caption_str, joggled_help_str, switch_select_form, n, od) =
            True then begin
          ...
          new_switch_info.joggle_length := 6;
          // joggle length (zero not valid).
      ...
      repeat
        i := alert(4, '    custom  switch  timbering ?',
          ...
          '', '', '', '?  help', 'no  -  let  Templot0  use  fixed  spacings',
          'yes  -  let  me  enter  the  spacings', 4);
        if i = 4 then
          alert_help(0, fixed_spacing_help_str, '');
      until i <> 4;

      if i = 6 then begin        // get spacings.
        repeat
          ...
          if ii = 3 then
            alert_help(0, front_timb_help_str, '');
        until ii <> 3;
        ...
        putdim('', 2, 'spacing back from TOE to first front ' + front_str + ' J1',
          ABS(new_switch_info.sleeper_j1), True, True, False, False);
        // no neg, no preset, zero ok, don't terminate on zero.
        putdim('', 2, 'spacing back from J1 to next front ' + front_str + ' J2',
          ABS(new_switch_info.sleeper_j2), True, True, False, False);          // ditto...
        putdim('', 2, 'spacing back from J2 to next front ' + front_str + ' J3',
          ABS(new_switch_info.sleeper_j3), True, True, False, False);          // ditto...
        putdim('', 2, 'spacing back from J3 to next front ' + front_str + ' J4',
          ABS(new_switch_info.sleeper_j4), True, True, False, False);          // ditto...
        ...
        temp_flag := getdims('custom  switch  front  spacings',
          front_timb_help_str, switch_select_form, n, od);
      end
      else
        temp_flag := False;
        ...
        repeat
          ...
          for m := 0 to 7 do
            n := putdim('Terminate this list of spacings by entering a zero.',
              2, 'spacing forward to timber number ' + IntToStr(
              i + m + 1), new_switch_info.timber_centres[(i + m)], True, True, False, True);
          // zero ok - terminates the list.
          if n <> 7 then
            run_error(212);
          if getdims('custom  switch  timber  spacings', '', switch_select_form, n, od) =
            True then begin
            if (new_switch_info.timber_centres[(i - 1)] <> 0) or (i = 1) then
              new_switch_info.timber_centres[i] := ABS(od[0])
            else
              BREAK;  // 0.91.c  "or (i=1)" bug fix to allow zero toe-to-timber (NER switches).
            if new_switch_info.timber_centres[i] <> 0 then
              new_switch_info.timber_centres[i + 1] := ABS(od[1])
            else
              BREAK;
            if new_switch_info.timber_centres[(i + 1)] <> 0 then
              new_switch_info.timber_centres[i + 2] := ABS(od[2])
            else
              BREAK;
            if new_switch_info.timber_centres[(i + 2)] <> 0 then
              new_switch_info.timber_centres[i + 3] := ABS(od[3])
            else
              BREAK;
            if new_switch_info.timber_centres[(i + 3)] <> 0 then
              new_switch_info.timber_centres[i + 4] := ABS(od[4])
            else
              BREAK;
            if new_switch_info.timber_centres[(i + 4)] <> 0 then
              new_switch_info.timber_centres[i + 5] := ABS(od[5])
            else
              BREAK;
            if new_switch_info.timber_centres[(i + 5)] <> 0 then
              new_switch_info.timber_centres[i + 6] := ABS(od[6])
            else
              BREAK;
            if new_switch_info.timber_centres[(i + 6)] <> 0 then
              new_switch_info.timber_centres[i + 7] := ABS(od[7])
            else
              BREAK;
          end
          else begin
            new_switch_info.timber_centres[i] := 0;
            // cancelled form, so terminate list right here.
            BREAK;
          end;
          Inc(i, 8);
        until i > (swtimbco_c - 8);   // list full (no space for another 8 plus end marker).

        new_switch_info.timber_centres[swtimbco_c] := 0;
        // ensure there is always an end marker.
      end
      ...
      Items.Strings[ItemIndex] := '  custom  ' + IntToStr(ABS(list_switch_info.group_code)) +
        ' :  ' + custom_str;     // put new name in list.
...
begin
  with switch_selector_listbox do begin
    ...
    if (switch_info_showing = True) and (data_child_form.Visible = True) then begin
    ...
    with switch_select_form.switch_selector_listbox.Items do begin
      ...
      for n := 0 to (Count - 1) do begin
        ...
        with Tswitch(Objects[n]).list_switch_info do begin
          ...
          if (group_code <> sw_group) or (size_code <> sw_size) then
            CONTINUE;
...
begin
  Result := False;      // default init.
  try
    n := get_switch_list_index(sw_group, sw_size);
    // get switch listbox index, or -1 if not found in list.
    if n < 0 then
      EXIT;
...
var
  ...
  begin
    ...
    for n := 0 to swtimbco_c do
      sw_init_info.timber_centres[n] := 0;    // switch timbering spacings.
    ...
    sw_init_info.num_bridge_chairs_main_rail := 0;
    // not used in experimental chairing  // byte 214a
    sw_init_info.num_bridge_chairs_turnout_rail := 0;
    // not used in experimental chairing  // byte 214a
  ...
  begin
    ...
    // which group of switches.   0.77.a  7-6-02.
    sw_init_info.size_code := sw_size;
    // size within group (0=empty slot, 1=shortest). 0.77.a  7-6-02.
    sw_init_info.group_count := sw_size_max;
    // number of switches in this group (max sw_init_info.size_code in this group, min size is always 1).
...
begin
  ...
  sw_init_info.old_size := 0;
  // no longer used, but needed if file is reloaded in old version (pre 0.77.a).
  ...
  sw_init_info.sleeper_j1 := 0 - 24.5;
  // sleeper J1 dimension (negative spacing from toe to first swith-front sleeper).
  ...
  sw_init_info.stock_rail := 534;
  // stock rail length.     ( 44'6"  error on Paddington R 3794a ?)
  sw_init_info.sleeper_j1 := 0 - 24.5;
  // sleeper J1 dimension (negative spacing from toe to first swith-front sleeper.
  sw_init_info.sleeper_j2 := 0 - 28;         // sleeper J2 dimension (negative spacing from J1).
  sw_init_info.sleeper_j3 := 0;            // sleeper J3 dimension (negative spacing from J2).
  ...
  add_to_list(2, 1, 6, '  BH  size  A  semi-curved  flexible  switch  ( REA )',
    'REA semi-curved  A-size');
  ...
  add_to_list(2, 2, 6, '  BH  size  B  semi-curved  flexible  switch  ( REA )',
    'REA semi-curved  B-size');
  ...
  add_to_list(2, 3, 6, '  BH  size  C  semi-curved  flexible  switch  ( REA )',
    'REA semi-curved  C-size');
  ...
  add_to_list(2, 4, 6, '  BH  size  D  semi-curved  flexible  switch  ( REA )',
    'REA semi-curved  D-size');
  ...
  add_to_list(2, 5, 6, '  BH  size  E  semi-curved  flexible  switch  ( REA )',
    'REA semi-curved  E-size');
  ...
  add_to_list(2, 6, 6, '  BH  size  F  semi-curved  flexible  switch  ( REA )',
    'REA semi-curved  F-size');
  ...
  sw_init_info.switch_radius_inchormax := max_rad;    // straight switch
  sw_init_info.switch_rail := 384;             // switch rail length.
  sw_init_info.stock_rail := 532;
  // stock rail length.       (44'4"   error on Paddington R 3794a ?)
  sw_init_info.sleeper_j1 := 0 - 24.5;
  // sleeper J1 dimension (negative spacing from toe to first swith-front sleeper).
  ...
  add_to_list(4, 1, 8, '  BH  GWR  old-type    9 ft.  heel  switch  (curved)',
    'GWR   9 ft. heel switch (curved)');
  ...
  add_to_list(4, 2, 8, '  BH  GWR  old-type  10 ft.  heel  switch  (curved)',
    'GWR  10 ft. heel switch (curved)');
  ...
  add_to_list(4, 3, 8, '  BH  GWR  old-type  12 ft.  heel  switch  (curved)',
    'GWR  12 ft. heel switch (curved)');
  ...
  add_to_list(4, 4, 8, '  BH  GWR  old-type  14 ft.  heel  switch  (curved)',
    'GWR  14 ft. heel switch (curved)');
  ...
  add_to_list(4, 5, 8, '  BH  GWR  old-type  15 ft.  heel  switch  (curved)',
    'GWR  15 ft. heel switch (curved)');
  ...
  add_to_list(4, 6, 8, '  BH  GWR  old-type  16 ft.  heel  switch  (curved)',
    'GWR  16 ft. heel switch (curved)');
  ...
  add_to_list(4, 7, 8, '  BH  GWR  old-type  18 ft.  heel  switch  (straight)',
    'GWR  18 ft. heel switch (straight)');
  ...
  add_to_list(4, 8, 8, '  BH  GWR  old-type  20 ft.  heel  switch  (straight)',
    'GWR  20 ft. heel switch (straight)');
  ...
  add_to_list(5, 1, 6, '  FB  size  SA  semi-curved  flexible  switch  ( BS-110A sidings)',
    'FB semi-curved SA');
  ...
  add_to_list(5, 2, 6, '  FB  size  SB  semi-curved  flexible  switch  ( FB-109 )',
    'FB semi-curved SB');
  ...
  add_to_list(5, 3, 6, '  FB  size  SC  semi-curved  flexible  switch  ( FB-109 )',
    'FB semi-curved SC');
  ...
  add_to_list(5, 4, 6, '  FB  size  SD  semi-curved  flexible  switch  ( FB-109 )',
    'FB semi-curved SD');
  ...
  add_to_list(5, 5, 6, '  FB  size  SE  semi-curved  flexible  switch  ( FB-109 )',
    'FB semi-curved SE');
  ...
  add_to_list(5, 6, 6, '  FB  size  SF  semi-curved  flexible  switch  ( FB-109 )',
    'FB semi-curved SF');
  ...
  add_to_list(6, 1, 6, '  FB  size  CA  curved  flexible  switch  ( BS-110A,  BS-113A )',
    'FB BS-110A curved CA');
  ...
  add_to_list(6, 2, 6, '  FB  size  CB  curved  flexible  switch  ( BS-110A,  BS-113A )',
    'FB BS-110A curved CB');
  ...
  add_to_list(6, 3, 6, '  FB  size  CC  curved  flexible  switch  ( BS-110A,  BS-113A )',
    'FB BS-110A curved CC');
  ...
  add_to_list(6, 4, 6, '  FB  size  CD  curved  flexible  switch  ( BS-110A,  BS-113A )',
    'FB BS-110A curved CD');
  ...
  add_to_list(6, 5, 6, '  FB  size  CE  curved  flexible  switch  ( BS-110A,  BS-113A )',
    'FB BS-110A curved CE');
  ...
  add_to_list(6, 6, 6, '  FB  size  CF  curved  flexible  switch  ( BS-110A,  BS-113A )',
    'FB BS-110A curved CF');
  ...
  *)
  ...
  add_to_list(7, 1, 3, '  BH  or  FB    1:24  ( A )  non-prototype  short  model  switch',
    ' 1:24  model-only');
  ...
  add_to_list(7, 2, 3, '  BH  or  FB    1:32  ( B )  non-prototype  short  model  switch',
    ' 1:32  model-only');
  ...
  add_to_list(7, 3, 3, '  BH  or  FB    1:40  ( C )  non-prototype  short  model  switch',
    ' 1:40  model-only');
  ...
  slip_switch_6 := add_to_list(8, 1, 4, '  BH  or  FB    1:24  ( A )  switch  for  1:6  slip',
    ' 1:24  ( 1:6  slip )');     // 215a  slip_switch_6  global   index for make_slip functions
  ...
  slip_switch_7 := add_to_list(8, 2, 4, '  BH  or  FB    1:32  ( B )  switch  for  1:7  slip',
    ' 1:32  ( 1:7  slip )');             // 215a
  ...
  slip_switch_8 := add_to_list(8, 3, 4, '  BH  or  FB    1:32  ( B )  switch  for  1:8  slip',
    ' 1:32  ( 1:8  slip )');            // 215a
  ...
  slip_switch_10 := add_to_list(8, 4, 4, '  BH  or  FB    1:40  ( C )  switch  for  1:10  slip',
    ' 1:40  ( 1:10  slip )');        // 215a
  //--------------------------------------
  ...
  sw_init_info.timber_centres[0] := 3.5;
  //  same as 12ft Standard Straight Switch - except first timber 3.5" from toe.
  sw_init_info.timber_centres[1] := 26;
  sw_init_info.timber_centres[2] := 26;
  sw_init_info.timber_centres[3] := 26;
  sw_init_info.timber_centres[4] := 26.5;
  ...
  sw_init_info.old_size := 6;
  // custom switch - no longer used, but needed if file is reloaded in old version (pre 0.77.a).
