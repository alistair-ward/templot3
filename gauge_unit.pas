...
const
  ...
  scale_help_str: string = '     Scale' +
    ...
    + '||Track dimensions which are usually modelled exactly to scale, for example rail lengths, are normally entered in INCHES full-size on the prototype.' + '||Track dimensions which are commonly modelled to a non-scale size, for example the track gauge, are normally entered in actual MILLIMETRES on the model.' + '||For the purpose of printing construction templates and track plans, a printed size of 100% means 100% of this model size.' + '||The maximum scale Templot0 will accept is 500 mm/ft or 1:0.6 (which is nearly twice full-size!).' + '||The minumum acceptable scale is 0.1 mm/ft or 1:3048.' + '||For engineering applications the scale should be entered as a ratio, for example 1:16 or 1:50.';
  ...
  gauge_help_str: string = '      Gauge / Scale  Selection  List' +
    ...
    '||To convert a selected group of background templates to the new gauge and scale setting, click the CONVERT GROUP button. This function ignores the MINT option box (see below).' + '||Click the SHOW INFO button to see full details of the dimensions which apply to the currently selected gauge.' + '||Click SET EXACT SCALE... if you want the track gauge, check-rail and flangeway dimensions' + ' to be scaled exactly from British full-size practice (bull-head rail).' + '  Or click one of the SET CUSTOM... buttons to enter your own custom dimensions.' + ' You can have up to 4 different custom settings available for use. These will appear at the bottom of the list.' + '||If the gauge which you are using has not yet been implemented in this upgrade version of Templot, please' + ' enter your requirements as a custom setting instead. More information is available by clicking the chat (=) button.' + '||If the MINT NEW option box is selected, the new control template at the new gauge and scale will be a mint template. If the MATCH ORIGINAL option box is selected,' + ' Templot0 will match the new control template at the new gauge and scale to the size and position of the original control template.' + '||These option settings also apply when using the TEMPLATE > GAUGE AND SCALE menu items, and can also be changed using the TEMPLATE > GAUGE AND SCALE > MINT NEW and MATCH ORIGINAL menu options.' + '||For an explanation of a mint template, select the TEMPLATE > QUICK SET... menu item and then click the ?HELP button on the QUICK SET window.' + '||If you close the form or click the CANCEL button the previous gauge and scale settings' + ' will remain unchanged.' + '||When this form first appears, it will show the gauge/scale setting corresponding to the control template on the trackpad. If there are any differences between the dimensions in use for the control template' + ' and the standard dimensions for this gauge/scale setting, a red warning label "N.B. Dimensions now in use differ" will appear above the list. Then clicking the SHOW INFO button will show the list of standard dimensions' + ' for this gauge/scale setting, and any which curently differ will be shown in RED.' + '||These dimensions can also be checked by selecting the TEMPLATE > GAUGE AND SCALE > DETAILS... menu item.' + '||Handy Hint:' + '|After reloading templates from a data file which you have not saved yourself (e.g. a downloaded file or one from another Templot0 user) it is sensible to check for any dimension differences before continuing to use the templates' + ' for your own designs (click the TEMPLATE > GAUGE AND SCALE > DETAILS... menu item).' + ' To restore the control template to the standard dimensions for this gauge and scale setting, simply click the green OK bar. To continue using the modified dimensions, close the form or click CANCEL.';
...
begin
  if help(-1, gauge_help_str, 'chat  -  more  information') = 1 then
    chat_click;
end;
...
begin
  if keeps_list.Count < 1 then
    EXIT;
  index := gauge_form.gauge_listbox.ItemIndex;
  // may get changed if the form is re-activated after help or alerts.
  ...
  gauge_form.gauge_listbox.ItemIndex := index;
  // may have been changed if the form is re-activated after help or alerts.
  gauge_list_click;

  if alert(7, '    convert  group  to  ' + Trim(gauge_form.ok_large_panel.Caption),
    'You are about to change the gauge/scale setting to ' + Trim(
    gauge_form.ok_large_panel.Caption) + ' for all the templates in the currently selected group.'
    + '||If these templates may be needed again they should first be saved (cancel this and click the SAVE GROUP button in the Storage Box).' + '||This CONVERT GROUP function does not change the control template to the new gauge/scale setting. To do that, click the OK button when this function finishes.' + ' To leave the control template unchanged, click the CANCEL button when this function finishes.' + '||This CONVERT GROUP function ignores the MINT tickbox.' + '||This function may take some time to complete.', '', '', '', '', 'cancel', 'O K    -    continue', 0) = 5 then
    EXIT;
...
begin
  with gauge_form do begin
    ...
    then begin
      if alert(2, '    ' + gauge[gauge_select_i].name_str_glist + '  not  yet  implemented',
        ...
        '', '', '', '', 'cancel  and  continue', 'chat  -  more  information', 0) = 6 then
        chat_click;
    ...
    then begin
      if alert(2, '                         ' + gauge[gauge_select_i].name_str_glist,
        ...
        , '', '', '', '', 'cancel', 'enter ' + gauge[gauge_select_i].name_str_glist +
        '  data', 0) = 6 then
        custom_click(gauge_select_i - (gauge_listbox.Items.Count - 5));
      EXIT;// form remains showing
    end;
    ...
    if (gauge_select_i < (gauge_listbox.Items.Count - 1)) and (gauge_select_i >
      (gauge_listbox.Items.Count - 6)) then
      custom_gauge := True
    else
      custom_gauge := False;
  ...
  if change_group = True then
    ...
    if not_mint = True then begin
      ...
      if gauge_form.retain_length_checkbox.Checked = True
      // 208d test added, was (0)
      then
        mint_new_current(1)
      else
        mint_new_current(0);
...
begin
  with gauge_form do begin
    if gauge_form.Showing = True then
    ...
    if gauge_select_i <> gauge_i then
...
begin
  ...
  then begin
    ...
    with gauge[gauge_i] do begin
      with cpi do begin
        ...
        with diff_flags do begin
          ...
          if ABS(scale_pi - scale_glist) > minfp then begin
            flags[0] := False;
            cur_vals[0] := scale_pi;
            same := False;
          end;  // mm/ft.
          if ABS(gauge_pi - gauge_glist) > minfp then begin
            flags[1] := False;
            cur_vals[1] := gauge_pi;
            same := False;
          end;  // mm.
          if ABS(fw_pi - fw_glist) > minfp then begin
            flags[2] := False;
            cur_vals[2] := fw_pi;
            same := False;
          end;  // mm flangeway.
          if ABS(fwe_pi - fwe_glist) > minfp then begin
            flags[3] := False;
            cur_vals[3] := fwe_pi;
            same := False;
          end;  // mm flangeway end (flangeway+flare).
          if ABS(trtscent_pi - trtscent_glist) > minfp then begin
            flags[4] := False;
            cur_vals[4] := trtscent_pi;
            same := False;
          end;  // mm track centres, turnout side.
          if ABS(trmscent_pi - trmscent_glist) > minfp then begin
            flags[5] := False;
            cur_vals[5] := trmscent_pi;
            same := False;
          end;  // mm ditto, main side.
          if ABS(min_radius_pi - min_radius_glist) > minfp then begin
            flags[6] := False;
            cur_vals[6] := min_radius_pi;
            same := False;
          end;  // mm minimum radius for check.
    ...
    if same = True then
      warning_panel.Hide                       // same.
    else
      warning_panel.Show;                      // different.
  end
  else
    warning_panel.Hide;
...
const
  chat_str: string = '    Gauge  and  Scale  Selection  List' +
    '||I have tried to make this list as comprehensive as possible, but it has not been possible to include any narrow-gauge settings because there are just too many variations.' + ' Narrow-gauge templates can be created as custom settings.' + '||If you are using a gauge or scale combination for standard-gauge not shown here I would be pleased to know about it, and the various dimensions which you have adopted.' + ' In the meantime you can enter them as a custom setting.' + '||If you can help to fill in any obvious gaps in this list I would be pleased to hear from you. The information needed is:' + '||  gauge (minimum) and scale' + '|  nominal flangeway gap and check rail flare-out (end gap)' + '|  minimum double-track centres' + '|  recommended minimum radius' + '||The gauge also needs a NAME. I have had to invent some of these because there seems to be no established name in use.' + '||Please note that for the EXACT scale setting the track gauge is assumed to be the traditional 56.5 inches.' + ' Some tracks in recent years have been set to 1432 mm gauge (about 3 mm less) - if you need this please enter your dimensions as a custom setting.' + '||Engineers coming across this list for the first time will no doubt be amazed' + ' at the weird scale ratios and mixture of metric and imperial dimensions that we go in for! All the gauges listed have been' + ' used by someone somewhere or referred to in print (if the internet counts as print).' + '||Except, of course, the T-55 Templot0 Startup gauge which is 5.5 mm/ft on 1" gauge with EM clearances. This is intended to be' + ' 100% fictitious - unless you know different !';
begin
  chat(chat_str);
end;
//_______________________________________________________________________________________
...
const
  help_str: string = '      Exact  Scale' +
    ...
    + '||If you are using a dead scale gauge (e.g. S4, ScaleSeven, etc.) you should select the appropriate size from the list, as there' + ' are slight variations in the standards to accommodate manufacturing tolerances ( although these are likely to show up only if you' + ' are printing at an enlarged scale on a very-high-resolution printer ).' + '||The maximum scale Templot0 will accept is 500 mm/ft or 1:0.6 (which is nearly twice full-size!). The minimum scale is 0.1 mm/ft or 1:3048.' + '||When you save and reload EXACT SCALE templates via your storage box, the exact scale settings for each one will be preserved, but will not' + ' be reflected here in the gauge list. Once you have such an EXACT SCALE control template on the trackpad, its settings will continue to be used for' + ' any subsequent templates until you come here and select another gauge size, or copy a template from the background or storage box, so there is no need to re-enter the EXACT SCALE setting for them unless' + ' you want to change the scale, or swap between different scales. The settings from an EXACT SCALE template can also be adopted into to one of the' + ' CUSTOM slots if you are not using them for other custom settings.';
...
begin
  repeat
    i := alert(4, '           exact  scale ...',
      ...
      + '||Please select your preference.', '', '', 'help',
      'scale  ratio  1 : n', 'cancel', 'scale  in  mm / ft', 3);
    case i of
      3:
      ...
      4: begin
        ...
        if n <> 0 then
          EXIT;
        if getdims('exact  scale  setting', scale_help_str, gauge_form, n, od) =
          True then begin
          gauge_select_i := gauge_listbox.Items.Count - 1;
          gauge_i := gauge_select_i;

          init_gauge_defaults(gauge_i);
          // first clear any previous exact setting.
      ...
      6: begin
        n := putdim(help_str, 1, 'EXACT  SCALE  in  mm / ft', scale, True, True, True, False);
        // no negative, no pre-set, no zero, don't terminate on zero.
        if n <> 0 then
          EXIT;
        if getdims('exact  scale  setting', scale_help_str, gauge_form, n, od) =
  ...
  until i <> 3;
  gauge[gauge_i].scale_glist := limits(0.1, 500, gauge[gauge_i].scale_glist, dummy_i);
  // maximum scale 500 mm to the foot (nearly twice full-size).
...
begin
  if (Key = Chr(13)) and (gauge_listbox.Focused = True) then
    ok_button.Click; //  ENTER key clicks OK.
end;
//______________________________________________________________________________________
...
begin
  gauge_listbox.Color := get_colour('choose  a  new  colour  for  the  gauge  list',
    gauge_listbox.Color);
end;
//____________________________________________________________________________________________
...
begin
  with gs do begin
    ...
    if gauge_glist = def_req then
      gauge_glist := 56.5 * inscale;        // default 4' 8.5" gauge.
    if fw_glist = def_req then
      fw_glist := 1.75 * inscale;              // default 1.75" dead-scale flangeway.
    if fwe_glist = def_req then
      fwe_glist := fw_glist + 1.75 * inscale;   // default +1.75" flare.
    if trtscent_glist = def_req then
      trtscent_glist := 134 * inscale;   // 11' 2" track centres turnout-side
    if trmscent_glist = def_req then
      trmscent_glist := 134 * inscale;   // ditto main-side
    if min_radius_glist = def_req then
      min_radius_glist := 152 * scale; //  152 ft. minimum radius warning.
...
begin
  ...
  for i := 0 to gauge_indexmax_c do
    init_gauge_defaults(i);   // first fill with defaults.
  ...
  gauge_form.gauge_listbox.Items.Add(listbox_str);
  Inc(i);
  if i > gauge_indexmax_c then
    run_error(39);
  ...
  {with}   gauge_form.gauge_listbox.Items.Add(listbox_str);
  Inc(i);
  if i > gauge_indexmax_c then
    run_error(39);
  ...
  with gauge[i] do begin
    listbox_str := ' H00-DN   16.2  mm    3.75 mm/ft  1:81.28   H0-00 hybrid   EMGS dims';
    // 212a
  ...
  with gauge[i] do begin
    listbox_str := ' 00-D0GAI 16.5  mm    4 mm/ft     1:76.2    00 D0GA Intermediate UK ';
    // added 212a
    scale_glist := 4;            // mm per ft.
    gauge_glist := 16.5;         // mm.
  ...
  with gauge[i] do begin
    listbox_str := ' 00-BF    16.5  mm    4 mm/ft     1:76.2    00 Commercial Fine UK   ';
    // 212a fw was 1.25
    scale_glist := 4;            // mm per ft.
    gauge_glist := 16.5;         // mm.
    fw_glist := 1.3;             // mm flangeway.
  ...
  with gauge[i] do begin
    listbox_str := ' EM-SF    18.0  mm    4 mm/ft     1:76.2    EM Special Fine         ';
    // added 212a
    scale_glist := 4;            // mm per ft.
    gauge_glist := 18.0;         // mm.
    fw_glist := 0.8;             // mm flangeway.
  ...
  with gauge[i] do begin
    listbox_str := ' 0-SF     31.20 mm    7 mm/ft     1:43.54   0 SPECIAL-FINE          ';
    // added 0.79.a
  ...
  listbox_str := ' CUSTOM-a                                custom settings (a)';
  gauge_form.gauge_listbox.Items.Add(listbox_str);
  Inc(i);
  if i > gauge_indexmax_c then
    run_error(39);
  listbox_str := ' CUSTOM-b                                custom settings (b)';
  gauge_form.gauge_listbox.Items.Add(listbox_str);
  Inc(i);
  if i > gauge_indexmax_c then
    run_error(39);
  listbox_str := ' CUSTOM-c                                custom settings (c)';
  gauge_form.gauge_listbox.Items.Add(listbox_str);
  Inc(i);
  if i > gauge_indexmax_c then
    run_error(39);
  listbox_str := ' CUSTOM-d                                custom settings (d)';
  gauge_form.gauge_listbox.Items.Add(listbox_str);
  Inc(i);
  if i > gauge_indexmax_c then
    run_error(39);

  listbox_str := ' EXACT                                      exact scale     ';
  gauge_form.gauge_listbox.Items.Add(listbox_str);

  gauge[i].scale_glist := 0;   // EXACT mm per ft.     // to be entered on gauge form.

  for j := 0 to i do begin
    temp_str := Trim(Copy(gauge_form.gauge_listbox.Items.Strings[j], 1, 10));
    // copy from within first 10 chars.
    ...
    if sc <> 0 then begin
      ...
      gauge[j].fwe_glist := gauge[j].fw_glist + 1.75 * ins;
      // 215a   use prototype flare angle for all (flangeway + 1.75" to end gap)
...
const
  help_all_str: string =
    '||With most standard gauge tracks you will probably only need to set the track gauge and flangeway gaps.'
    + ' For the remaining dimensions the pre-sets will usually give satisfactory results (enter a "/" slash).'
    + '||The existing dimensions showing in the input box are Templot0''s best guess based on the current gauge/scale combination, so it is' + ' useful to select the nearest existing size before entering your custom dimensions.' + '||When you save and reload templates which use these custom settings via your storage box, the custom settings for each one will be preserved, but will not' + ' be reflected here in the gauge list.' + '||Once you have such a custom control template on the trackpad, its settings will continue to be used for' + ' any subsequent templates until you come here and select another gauge size, or copy a different template from the background or storage box.' + '||So there is no real need to adopt the custom settings for them into this list. But doing so makes it convenient to swap between several set-ups without having to remember which template to copy or reload.' + '||Or in other words, do this:' + '||1. Enter here your required custom setting dimensions.' + '||2. Click OK to create a template using them.' + '||3. Save the template via your storage box.' + '||4. When you next need to use these settings, simply reload this template as a starting point.' + '||5. There is no need to come here again until you want to create a different custom setting.' + '||6. But you can if you want to, and simply adopt the settings into one of the custom slots. It is not necessary to re-enter the actual dimensions.';

  help_adopt_str: string =
    '||If the control template dimensions have been modified from a standard gauge/scale setting you can adopt these dimensions in their entirety' + ' to create your new custom gauge/scale setting.' + '||( If the control template dimensions have not been modified since a standard setting was selected, adopting them will simply duplicate the standard setting, to no advantage.)' + '||When you store (and/or then save) a template using modified dimensions, these dimensions are retained and will be restored when you reload and copy the template back from your storage box.' + ' And unless you change the dimensions, they will continue to be used for any subsequent templates.' + '||So the need to make them a custom setting is not immediately obvious. But doing so makes it' + ' convenient to swap between several set-ups without having to remember which template to copy or reload.' + '||For example, the standard gauge/scale settings include the standard dimension for adjacent track centres (normally based on 6ft way between the rails,' + ' i.e. 11ft 2ins centres for standard gauge running lines).' + '  You could create an alternative custom setting for loops and sidings (9ft or 10ft way), and quickly swap between the two without having to re-enter the dimensions or reload templates.';
  ...
  help_cen_str: string = '    Adjacent  Track  Spacing.' +
    ...
    + '||Separate templates for these adjacent tracks are created by selecting the TOOLS > MAKE CROSSOVER or TOOLS > MAKE DOUBLE-TRACK menu items.' + '||Adjacent track rails can also be placed on the control template as planning aids, by selecting the GENERATOR > ADJACENT TRACKS menu items. It is necessary to select each rail separately.' + '||The minimum spacing on straight track should normally give a 6ft way between the inner rails, i.e. 11ft 2in (134in) (scale) minimum centre-to-centre spacing for standard-gauge tracks.' + '||Where there are sharp curves or superelevation, this distance must be increased to allow for vehicle overhang.' + '||The minimum spacing should also be increased when using the TOOLS > MAKE DOUBLE-TRACK functions on a transition curve, as it is not then mathematically possible to create an exact uniformly spaced adjacent track.' + '||Where the adjacent track forms a loop or siding, the spacing should normally be increased by 4ft to 15ft 2in (182in) (scale).' + ' This is to provide a space for signal posts and other obstructions, and to ensure the safety of shunting staff.' + '||If preferred, full-size dimensions can be entered (in inches) by using the P conversion factor. For example, to enter the scale equivalent of 15ft 2in, enter p182.' + ' For more information about conversion factors, click the ?HELP button.' + '||Many model railway gauge standards incorporate overscale nominal spacings to allow for the use of sharp curves.' + '||In constructing a full track plan these spacing dimensions may frequently need to be modified for individual templates. This is done using he GEOMETRY > ADJACENT TRACK CENTRES... menu item.';
  ...
  help_gap_str: string = '      Flangeway  Gap' +
    '||Enter a dimension in mm on the model for the flangeway gap between the nose of the crossing and the wing rails, and also between the running rail and the check rails.' + '||The preset dimension is the scale equivalent of 1.75" full-size.';

  help_end_gap_str: string = '      Flangeway  End  Gap' +
    '||Enter a dimension in mm on the model for the flangeway end gap between the running rail and the flared ends of check and wing rails.' + '||The preset dimension is the scale equivalent of 3.5" full-size.';

  help_rad_str: string = '      Warning  Radius' +
    '||Enter a dimension in mm on the model for the radius (at the track centre-line) below which the warning lamp in the INFORMATION panel will flash red.' + '||The preset dimension is the scale equivalent of 152ft radius full size.';
...
begin
  ...
  with math_form do begin
    ...
    if gauge[index].scale_glist <> 0 then begin
      math_editbox.Text := gauge[index].name_str_glist;

      i := alert(4, '    custom (' + dummy_str + ')  gauge / scale ...',
        'The custom (' + dummy_str + ') gauge/scale setting is already in use as ' +
        gauge[index].name_str_glist + '||Do you want to delete it or replace it?',
        '', '', '', 'delete  ' + gauge[index].name_str_glist, 'cancel',
        'replace  ' + gauge[index].name_str_glist, 0);
    ...
    big_label.Caption := insert_crlf_str('|||Enter a short designation for this custom (' +
      dummy_str + ')  gauge/scale setting.||Maximum 9 characters with no spaces.'
      +
      '||For example a designation EM-SDNGS might be for EM Gauge track in sidings, with an increased adjacent track spacing and a smaller radius warning limit.' + '||0-14MM might be for 14mm narrow-gauge (2ft gauge) in 7mm/ft scale.' + '||Do not use long words or spaces - maximum is 9 characters.' + '||Do not use a designation name which is already in use in the list.');
    ...
    if Trim(math_editbox.Text) <> '' then begin
    ...
    200:
      ...
      if name_str = gauge[n].name_str_glist then begin
        ShowMessage('error - The designation ' + name_str + ' is already in use.' +
        #13 + #13 + 'Please enter a different designation.');
        math_editbox.Text := '';
        goto 100;
      end;
    Inc(n);

    if n <= gauge_indexmax_c then
      goto 200;
  ...
  repeat
    i := alert(4, '    ' + name_str + '  gauge / scale ...',
      'Do you want to adopt the current control template dimensions for this ' +
      name_str + ' custom setting?', '', '', 'more  information',
      'yes  -  adopt  the  control  template  dimensions', 'cancel',
      'no  -  enter  custom  settings  individually', 3);
    case i of
      3:
        alert_help(0, '    ' + gauge[index].name_str_glist + '  Settings' + help_adopt_str +
          '||Custom  Settings' + help_all_str, '');
      4: begin
        with gauge[index] do begin
          with cpi do begin
        ...
        gauge_form.warning_panel.Visible := False;
        // just loaded, so there can't be any mismatch.
  ...
  if got_all_custom = False then begin

    repeat
      i := alert(4, '    ' + name_str + '  gauge / scale ...',
        'The scale size for this ' + name_str +
        ' custom setting can be entered in mm/ft, or as a scale ratio.' +
        '||Please select your preference.', '', '', 'help',
        'scale  ratio  1 : n', 'cancel', 'scale  in  mm / ft', 3);
      case i of
        3:
        ...
        4: begin
          n := putdim(scale_help_str, 0, 'scale  ratio  1 :', 304.8 /
            old_scale, True, True, True, False);
          // no negative, no pre-set, no zero, don't terminate on zero.   {mods 21-3-01, 0.68.b}
          if n <> 0 then
            run_error(231);
          if getdims('scale  ratio', '', gauge_form, n, od) = True
          then
            new_scale := ABS(304.8 / od[0])  //  1 foot = 304.8 mm
          else
            EXIT;
        end;
        ...
        6: begin
          n := putdim(scale_help_str, 1, 'scale  in  mm / ft',
            old_scale, True, True, True, False);
          // no negative, no pre-set, no zero, don't terminate on zero.  {mods 21-3-01, 0.68.b}
          if n <> 0 then
            run_error(232);
    ...
    new_scale := limits(0.1, 500, new_scale, dummy_i);
    // maximum scale 500 mm to the foot (nearly twice full-size).

    with cpi do begin // use existing control template settings. {mods 21-3-01, 0.68.b}

      putdim(help_gauge_str, 1, 'track  gauge',
        gauge_pi * new_scale / old_scale, True, False, True, False);
      // no negative, pre-set ok, no zero, don't terminate on zero.
      putdim(help_gap_str, 1, 'flangeway  gap',
        fw_pi * new_scale / old_scale, True, False, False, False);
      // no negative, pre-set ok, zero ok, don't terminate on zero.
      putdim(help_end_gap_str, 1, 'flangeway  end gap ( at  flared  ends )',
        fwe_pi * new_scale / old_scale, True, False, False, False);
      // no negative, pre-set ok, zero ok, don't terminate on zero.

      putdim(help_cen_str, 1, 'adjacent  track  centres  TS ( turnout-side )',
        trtscent_pi * new_scale / old_scale, True, False, True, False);
      // no negative, pre-set ok, no zero, don't terminate on zero.
      putdim(help_cen_str, 1, 'adjacent  track  centres  MS ( main-side )',
        trmscent_pi * new_scale / old_scale, True, False, True, False);
      // no negative, pre-set ok, no zero, don't terminate on zero.
      n := putdim(help_rad_str, 1, 'minimum  radius  ( for  warning )',
        min_radius_pi * new_scale / old_scale, True, False, True, False);
      // no negative, pre-set ok, no zero, don't terminate on zero.
    ...
    if n <> 5 then
      EXIT;
    if getdims(name_str + '  custom  settings', help_str, gauge_form, n, od) =
      True then begin
      with gauge[index] do begin
  ...
  if gauge[index].scale_glist > minfp then begin
    ...
    dummy_str := StringReplace(dummy_str, 'S7(I)    ', Copy(name_str + '         ', 1, 9),
      [rfIgnoreCase]);

    dummy_str := StringReplace(dummy_str, '36.75 mm   ', FormatFloat(
      '00.00', gauge[index].gauge_glist) + ' mm   ', [rfIgnoreCase]);

    dummy_str := StringReplace(dummy_str, ' 7 mm/ft     ', FormatFloat(
      '00.00', gauge[index].scale_glist) + ' mm/ft  ', [rfIgnoreCase]);

    dummy_str := StringReplace(dummy_str, ' 0', '  ', [rfReplaceAll, rfIgnoreCase]);
    // remove leading zeroes

    dummy_str := StringReplace(dummy_str, '0 mm', '  mm', [rfReplaceAll, rfIgnoreCase]);
    // remove trailing zeroes

    case custom of

      0:
        dummy_str := dummy_str + 'a)';
      1:
        dummy_str := dummy_str + 'b)';
      2:
        dummy_str := dummy_str + 'c)';
      3:
        dummy_str := dummy_str + 'd)';

    end;//case

    gauge_form.gauge_listbox.Items.Strings[index] := ' ' + dummy_str;
    // update listbox line
  ...
  if gauge_form.Showing = True then
    gauge_form.ok_button.SetFocus;   // 0.93.a
end;
//________________________________________________________________________________________
...
begin
  ...
  with gauge_form do begin
    ...
    if (i < 0) or (i > (gauge_listbox.Items.Count - 1)) then
      EXIT;  // ???

    if gauge[i].scale_glist < minfp                     // entry nyi.
    then begin

      if i < (gauge_listbox.Items.Count - 5) then begin
        if i = 0 then
          no_data_str := insert_crlf_str(' (invalid dummy item)'
            +
          ...
          no_data_str := insert_crlf_str(ok_large_panel.Caption + sorry_str +
            '| Please click the chat button for more information.');
      end
      else begin
        if i < (gauge_listbox.Items.Count - 1) then
          no_data_str := insert_crlf_str(ok_large_panel.Caption +
            '|| No dimensions for this custom setting have yet been'
            +
      ...
      with info_listbox.Items do begin
        with gauge[i] do begin
          Add(space_lead(FormatFloat('00000.00', scale_glist)) +
            '  mm  SCALE per ft.  ( RATIO 1:' + round_str(304.8 / scale_glist, 2) + ' )');
          Add(space_lead(FormatFloat('00000.00', gauge_glist)) +
            '  mm  GAUGE between running rails.');
          Add(space_lead(FormatFloat('00000.00', fw_glist)) + '  mm  flangeway gap.');
          Add(space_lead(FormatFloat('00000.00', fwe_glist)) +
            '  mm  flangeway end gap (check rail flared ends).');
          Add(space_lead(FormatFloat('00000.00', trtscent_glist)) +
            '  mm  adjacent track centres, turnout side.');
          Add(space_lead(FormatFloat('00000.00', trmscent_glist)) +
            '  mm  adjacent track centres, main side.');
          Add(space_lead(FormatFloat('00000.00', min_radius_glist)) +
            '  mm  minimum radius limit for warnings.');
        end;//with
      end;//with
    end;
  end;//with form
...
begin
  if help(0, gauge_help_str, 'chat  -  more  information') = 1 then
    chat_click;
end;
//_________________________________________________________________________________________
...
procedure Tgauge_form.info_listboxDrawItem(Control: TWinControl; Index: Integer;
  Rect: TRect; State: TOwnerDrawState);
...
begin
  ...
  with info_listbox.Canvas do begin
    ...
    if (odSelected in State) = True then
      ...
      Brush.Color := info_listbox.Color;     // (yellow)

    FillRect(Rect);    { clear the rectangle }
    if (diff_flags.flags[Index] = True) or (warning_panel.Visible = False) or
      (gauge[gauge_listbox.ItemIndex].scale_glist = 0) then begin
      if Brush.Color = clBlack then
        Font.Color := info_listbox.Color  // (invert) selected for copying
      else
        Font.Color := clBlack;
    end
    else begin
