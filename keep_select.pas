...
begin
  ...
  try
    ...
    if not echo_cut then begin
      ...
      if not bgnd_clicked_in_quick_mode then begin
        // clean out any old deletedKee
    ...
    if bf then begin
      // first remove it from the background and free drawing data...
      if not wipe_it(n) then begin
    ...
    with keep_form.keep_image.Canvas do begin
      ...
      then begin
        for aq := rdMainRoadCentreLine to rdTurnoutRoadCentreLine do begin
          ...
          if (plain_track = False) or (aq = rdMainRoadCentreLine)
      ...
      then begin
        ...
        aq := rdAdjTrackTurnoutSideNearGaugeFace;
        draw_rail(aq);
        //                            end;
        ...
        //                       then begin                         // data is always in list so can print infilled
        aq := rdAdjTrackMainSideNearGaugeFace;
        draw_rail(aq);
        //                            end;
      ...
      if bgkeeps_form.gauge_faces_checkbox.Checked = True then
        for aq := rdStraightStockGaugeFace to rdTurnoutSideCheckGaugeFace do
        begin                      // main rails gauge faces
          if (plain_track = False) or (aq = rdStraightStockGaugeFace) or
            (aq = rdCurvedStockGaugeFace) // stock rails only, if plain track
          then
            draw_rail(aq);
        end;//next aq
      ...
      if bgkeeps_form.outer_edges_checkbox.Checked = True then
        ...
        begin                     // main rails outer edges
          if (plain_track = False) or (aq = rdStraightStockOuterFace) or
            (aq = rdCurvedStockOuterFace) then
            draw_rail(aq);
        end;//next aq
      ...
      if (plain_track = False) and (bgkeeps_form.gauge_faces_checkbox.Checked = True) and
        ...
        if (half_diamond = True) and (fixed_diamond = True) then begin
          ...
          mark_end(rdKCrossingCheckMainSideGaugeFace, 1, rdKCrossingCheckMainSideOuterEdge,
...
(*
  if (keep_form.Active) and (not control_template_on_save) then begin
    ...
    if control_template_on_save then begin
      ...
      if {check_if_abandoned<>-1} abandon_calcs then
        EXIT;
    end
    else begin
      ...
      if not calcs_done_and_valid then begin
        //  calcs not valid.
        alert(6, '    nothing  to  store',
          '|||    There is no valid template to be stored.',
          '', '', '', '', 'cancel', '', 0);
...
begin
  ...
  if keeps_list.Count > 0 then begin
    with keep_form do begin
      ...
      try
        copy_keep(keeps_list[list_index]);    // make the keep data current.
...
begin
  save_box(0, eSB_SaveBackground, eSO_Normal, '');         // don't need result flag.
end;
//_________________________________________________________________________________________
...
begin
  save_box(0, eSB_SaveUnused, eSO_Normal, '');         // don't need result flag.
end;
//________________________________________________________________________________________
...
begin
  save_box(0, eSB_SaveLibrary, eSO_Normal, '');         // don't need result flag.
end;
//______________________________________________________________________________________
...
begin
  ...
  save_box(0, eSB_SaveGroup, eSO_Normal, '');        // don't need result flag.
end;
//____________________________________________________________________________________________
