...
begin
  ...
  pi.turnoutTimberWidthInches := cpi.tbwide_pi;
  pi.sleeperWidthInches := cpi.slwide_pi;
  pi.maxTimberSpacingInches := cpi.ftimbspmax_pi;
  pi.sleeperLength := cpi.tb_pi;
  pi.mainsideEnds := cpi.mainside_ends_pi;
  pi.sleeperWidthAtRailJointInches := cpi.jt_slwide_pi;
  ...
  pi.railHeightInches := cpi.rail_height_pi;
  pi.seatThickInches := cpi.seat_thick_pi;
  pi.railInclination := cpi.rail_inclination_pi;
  pi.footHeightInches := cpi.foot_height_pi;
  pi.chairOutLengthInches := cpi.chair_outlen_pi;
  pi.chairInLengthInches := cpi.chair_inlen_pi;
  pi.chairWidthInches := cpi.chair_width_pi;
  pi.chairCornerRadiusInches := cpi.chair_corner_pi;
...
begin
  ...
  cpi.tbwide_pi := pi.turnoutTimberWidthInches;
  cpi.slwide_pi := pi.sleeperWidthInches;
  cpi.ftimbspmax_pi := pi.maxTimberSpacingInches;
  cpi.tb_pi := pi.sleeperLength;
  cpi.mainside_ends_pi := pi.mainsideEnds;
  cpi.jt_slwide_pi := pi.sleeperWidthAtRailJointInches;
  ...
  cpi.ck_ms_working1_pi := pi.checkRailLengthMainSide1Inches;
  cpi.ck_ms_working2_pi := pi.checkRailLengthMainSide2Inches;
  cpi.ck_ms_working3_pi := pi.checkRailLengthMainSide3Inches;
  cpi.ck_ms_ext1_pi := pi.checkRailExtensionMainSide1Inches;
  cpi.ck_ms_ext2_pi := pi.checkRailExtensionMainSide2Inches;
  cpi.wing_ms_reach1_pi := pi.wingRailReachMainSide1Inches;
  cpi.wing_ms_reach2_pi := pi.wingRailReachMainSide2Inches;
  cpi.railbottom_pi := pi.railBottom;
  cpi.rail_height_pi := pi.railHeightInches;
  cpi.seat_thick_pi := pi.seatThickInches;
  cpi.rail_inclination_pi := pi.railInclination;
  cpi.foot_height_pi := pi.footHeightInches;
  cpi.chair_outlen_pi := pi.chairOutLengthInches;
  cpi.chair_inlen_pi := pi.chairInLengthInches;
  cpi.chair_width_pi := pi.chairWidthInches;
  cpi.chair_corner_pi := pi.chairCornerRadiusInches;
