...
var
  ...
  print_busy: boolean = False;
  // checked by endless loop to avoid changing data in mid-print.

  want_bottom_margin: boolean = False;
  // bottom margin on last page not wanted unless multiple print runs.
...
// print the info page on banner paper.
...
begin
  with print_form do begin
    ...
    // can't click this once printing under way.
...
begin
  with print_form do begin
    ...
    if (banner_paper = True) and (printer_printing = False) then begin
      banner_fill_checkbox.Enabled := True;
      // might have been disabled if this is a change.
...
begin
  ...
  if colour = virtual_black_colour then
    Result := virtual_black_colour
  else
    Result := 0;
  ...
  try
    ...
    mod_col := (50 - print_intensity) * 5;
    // *5 arbitrary (gives range +250 to -250 as colour modifiers for intensity range 0-100).
  ...
  if (colour = virtual_black_colour) and (Result = clBlack) then
    Result := virtual_black_colour;  // don't set black for rail edges. (HP bug)
end;
...
begin
  if black_white = True then begin
    ...
    print_intensity := limits(0, 100, print_form.print_intensity_trackbar.Position * 2, dummy);
    // trackbar 0-50, normal 25. print_intensity 0-100, normal 50.
    ...
    if print_form.black_edges_checkbox.Checked = True then
      print_railedge_colour := virtual_black_colour;

    if print_railedge_colour = 0 then
      print_railedge_colour := virtual_black_colour;
    // HP bug. (0=black, don't mix with colours).
...
var
  ...
  begin
    ...
    if (adjust = True) and (pad_form.adjust_line_thickness_menu_entry.Checked = True) then
...
var
  ...
  max_sheet_down: array[0..sheet_across_c] of integer;
  // 0.93.a keep track of highest non-empty page in each row.
  ...
  begin
    with print_form do begin
      ...
      if banner_paper = False then begin
        Printer.EndDoc;
        // any further single sheet printing uses file assignment printing on a fresh page.
        printer_printing := False;

        if (info_checkbox.Checked = True) and (print_entire_pad_flag = False) and
          (no_cancel = True) then begin
        ...
        if (info_checkbox.Checked = True) and (print_entire_pad_flag = False) and
          (no_cancel = True) then
          print_info_banner(top_str, bottom_str);

        Printer.EndDoc;              // banner finished.
        printer_printing := False;
      end;

      if (no_cancel = True) and (index_sheet_wanted = True) then
      ...
      for i := 0 to (mark_index - 1) do begin
        ...
        then begin
          ...
          if mark_code <> eMC_99_TimberNumber then begin
            ...
            if impact > 0 then
              Pen.Width := 1;             // overide for impact printer or plotter.
            ...
            if (pad_form.print_timber_numbering_menu_entry.Checked = True) or
              ((out_factor > 0.99) and
              ...
              if (pad_form.timber_numbering_on_plain_track_menu_entry.Checked =
                False)   // 208a
                and (num_str <> 'A1') then begin
                s := Copy(num_str, 1, 1);

                if (s = 'A') or (s = 'E') or (s = 'R') or (s = 'N')
                // not wanted on plain track,
                then
                  CONTINUE;
              end;

              if check_limit(False, False, move_to) = True then begin
                Font.Assign(print_timber_numbers_font);

                if pad_form.scale_timber_numbering_menu_entry.Checked = True then begin
                  fontsize :=
                    Font.Size * out_factor;
                  if fontsize < 4 then
                    CONTINUE;      // minimum to be legible.
          ...
          then begin
            ...
            if (check_limits(infill_points[0], infill_points[1]) = True) and
              ...
              if black_white = True then
                ...
                if impact > 0 then
                  Brush.Color := printtimber_colour            // colour plotter.
                else
              ...
              case print_timb_infill_style of
                ...
                3:
                  if (impact > 0) or (black_white = True) then
                    CONTINUE
                  // 209c now no fill   was Brush.Style:=bsFDiagonal    // impact printer or plotter, or printing black and white.
                  else
                    Brush.Style := bsSolid;
          ...
          case mark_code of     // switch labels 206b
            ...
            eMC_701_XingFPLabel .. eMC_703_XingLabelEnd: begin
              ...
              if check_limit(False, False, move_to) = True then begin
                ...
                if scale > 3 then
                  Font.Size := Font.Size + 1; // a bit bigger above 3mm/ft
                ...
                TextOut(move_to.X - (TextWidth(switch_label_str) div 2),
                  // div 2 allows for rotation of template
  ...
  function get_w_dots(q: ERailData; n: integer): integer;
  ...
  function get_l_dots(q: ERailData; n: integer): integer;
  ...
  procedure draw_outline_railedge(aq: ERailData; pencol: integer);

  var
    now: integer;

  begin
    if ((plain_track = False) or (aq in rdStockRails) or (aq in rdAdjacentTracks)) and
      (aqyn[aq] = True) then begin
      with Printer.Canvas do begin
        Pen.Color := pencol;
        Pen.Mode := pmCopy;
        Pen.Style := psSolid;
  ...
  procedure draw_fill_rail;    // draw a complete filled rail.
  ...
  var
    ...
    begin
      ...
      if (start_index >= 0) and (start_index <= dots_index) and (stop_index >= 0) and
        (stop_index <= dots_index) then begin
  ...
  begin
    with Printer.Canvas do begin
      ...
      if (rail = rdAdjTrackTurnoutSideNearGaugeFace) or
        (rail = rdAdjTrackMainSideNearGaugeFace)   // 0.93.a platforms
      then
      ...
      if ((plain_track = False) or (aq in rdStockRailGaugeFaces) or (aq in rdAdjacentTracks))

      // if plain track, stock rails and adjacent tracks only.

      then begin
        if (aqyn[aq] = False) or (aqyn[aq_oppositeFace[aq]] = False)
        // data not for both edges?
        then begin
          if aqyn[aq] = True then
            draw_outline_railedge(aq, Pen.Color);
          if aqyn[aq_oppositeFace[aq]] = True then
            draw_outline_railedge(aq_oppositeFace[aq], Pen.Color);
          EXIT;
        end;
        ...
        then begin
          case aq of
            ...
            rdCurvedTurnoutWingGaugeFace: begin                         // ditto
        ...
        aq := aq_oppositeFace[rail];             // outer-edges.
        ...
        if (black_white = True) or (impact > 0) then begin
          Brush.Color := clWhite;
          ...
          then begin
            ...
            case print_platform_infill_style of
              ...
              3:
                if mapping_colours_print < 0          // solid.
                then
                  Brush.Style := bsBDiagonal
            ...
            if ((draw_ts_trackbed_cess_edge = True) and (rail =
              rdAdjTrackTurnoutSideFarGaugeFace)) or ((draw_ms_trackbed_cess_edge = True) and
              (rail = rdAdjTrackMainSideFarGaugeFace))
            // 215a
            then begin
        ...
        if dots_index > 2 then begin
          ...
          if ((half_diamond = False) or (fixed_diamond = False)) and
            ((rail = rdStraightTurnoutWingGaugeFace) or (rail =
            rdCurvedTurnoutWingGaugeFace)) then
          ...
          if (rail = rdStraightStockGaugeFace) or (rail = rdCurvedStockGaugeFace) or
            (rail = rdAdjTrackTurnoutSideFarGaugeFace) or
            (rail = rdAdjTrackMainSideFarGaugeFace)
          // 18,22 added 206b
          then begin
            modify_rail_end(
          ...
          then begin
            ...
            if (rail = rdAdjTrackTurnoutSideNearGaugeFace) and
              (draw_ts_platform = True) and (draw_ts_platform_rear_edge = False)
            // 0.93.a TS platform start
            then
              draw_outline_railedge(rdAdjTrackTurnoutSideNearGaugeFace, blanking_colour);
            // blank rear edge

            if (rail = rdAdjTrackMainSideNearGaugeFace) and (draw_ms_platform = True) and
              (draw_ms_platform_rear_edge = False)   // 0.93.a TS platform start
            then
              draw_outline_railedge(rdAdjTrackMainSideNearGaugeFace, blanking_colour);
            // blank rear edge

            // 0.93.a blank platform ends ...

            if (rail = rdAdjTrackTurnoutSideNearGaugeFace) and
              (draw_ts_platform = True) and (draw_ts_platform_start_edge = False)
            // 0.93.a TS platform start
            then
              modify_rail_end(0, dots_index, edge_colour, blanking_colour);

            if (rail = rdAdjTrackTurnoutSideNearGaugeFace) and
              (draw_ts_platform = True) and (draw_ts_platform_end_edge = False)
  ...
  begin
    ...
    if (aqyn[rdVeePointGaugeFace] = False) or (aqyn[rdVeeSpliceGaugeFace] = False) or
    ...
    begin
      ...
      // draw outline vee...
      if aqyn[rdVeeSpliceGaugeFace] = True then
        draw_outline_railedge(rdVeeSpliceGaugeFace, printcurail_colour);
      if aqyn[rdVeePointOuterFace] = True then
        draw_outline_railedge(rdVeePointOuterFace, printcurail_colour);
      if aqyn[rdVeeSpliceOuterFace] = True then
        draw_outline_railedge(rdVeeSpliceOuterFace, printcurail_colour);
    end
    else begin
      dots_index := 0 - 1;   // first increment is to zero.

      aq := rdVeePointGaugeFace;
      edge_started := False;
      ...
      with Printer.Canvas do begin
        ...
        Pen.Color := printcurail_colour;
        //  1 = virtual black. Bug in HP printer driver if black (0) specified.
  ...
  procedure mark_end(aq1: ERailData; aq1end: integer; aq2: ERailData; aq2end: integer);
  // make a rail end mark
  ...
  begin
    if plain_track = False then begin
      ...
      mark_end(rdStraightTurnoutWingGaugeFace, 1, rdStraightTurnoutWingOuterFace, 1);
      // turnout rail wing rail finish.
      mark_end(rdCurvedTurnoutWingGaugeFace, 1, rdCurvedTurnoutWingOuterFace, 1);
      // main rail wing rail finish.

      mark_end(rdMainSideCheckGaugeFace, 0, rdMainSideCheckOuterFace, 0);
      // main side check rail start.
      mark_end(rdMainSideCheckGaugeFace, 1, rdMainSideCheckOuterFace, 1);
      // main side check rail finish.

      mark_end(rdTurnoutSideCheckGaugeFace, 0, rdTurnoutSideCheckOuterFace, 0);
      // turnout side check rail start.
      mark_end(rdTurnoutSideCheckGaugeFace, 1, rdTurnoutSideCheckOuterFace, 1);
      // turnout side check rail finish.

      mark_end(rdVeePointGaugeFace, 0, rdVeeSpliceGaugeFace, 0);    // blunt nose.

      if (half_diamond = True) and (fixed_diamond = True)
      // planed faced of point rails for a fixed-diamond.
      then begin
        mark_end(rdStraightTurnoutWingGaugeFace, 0, rdStraightTurnoutWingOuterFace, 0);
        mark_end(rdCurvedTurnoutWingGaugeFace, 0, rdCurvedTurnoutWingOuterFace, 0);

        mark_end(rdKCrossingCheckMainSideGaugeFace, 1, rdKCrossingCheckMainSideOuterEdge, 1);
  ...
  // 0.93.a   pause banner ..
  begin
    if (banner_paper = True) and (all_pages = True) and (sheet_across = max_sheet_across) and
      (sheet_down = max_sheet_down[max_sheet_across]) and
      (print_form.banner_pause_final_page_checkbox.Checked = True) then
      ShowMessage('Banner printing.' + #13 + #13 + 'Now paused before sending page ' +
        Chr(sheet_across + 97) + '/' + IntToStr(sheet_down + 1) + ' to the printer.' +
        #13 + #13 + 'This will be the final printed page.' + #13 + #13 +
        'Click OK when ready to send it to the printer.');
  end;
  //////////////////////////////////////////////////////////////////
  ...
  begin
    if bgnd_form.output_shapes_in_front_of_sb_checkbox.Checked = True then
      print_sketchboard_items(Printer.Canvas, grid_left, grid_top);  // 206e

    print_bgnd_shapes(grid_left, grid_top);    // print all background shapes

    if bgnd_form.output_shapes_in_front_of_sb_checkbox.Checked = False then
      print_sketchboard_items(Printer.Canvas, grid_left, grid_top);  // 206e
...
begin
  ...
  try
    123:                            // may want to come back if banner infill changes.
      ...
      if Printer.Printing = True then begin
        alert(6, '    printer  busy',
        '||||The printer is currently in use.' + '||Please try again later.',
        '', '', '', '', '', 'O K', 0);
        EXIT;
      end
      else begin
    ...
    print_form.printer_label.Caption := printer_list.Strings[prindex];
    // put printer name on print form.
    cur_cal := Tprint_cal(printer_list.Objects[prindex]).cal_data;
    cal_ok := cur_cal.printer_calibrated;                          // flag for warning needed.
    impact := cur_cal.printer_impact;
    // use thin lines for impact printer or plotter.

    if banner_paper = True then begin
      print_form.banner_fill_checkbox.Enabled := True;
      // might have been disabled if this is a change.
      print_form.banner_pause_final_page_checkbox.Enabled := True;   // 0.93.a
    end;

    if print_preview(False, False, 0) = False
      ...
      with Canvas do begin
        ...
        for sheet_across := 0 to sheet_across_c do begin
          ...
          for sheet_down := 0 to sheet_down_c do begin              // every sheet in row.
            ...
            max_sheet_down[sheet_across] := sheet_down;
            // 0.93.a keep track of highest non-empty page in each row.
        ...
        if page_count > 1 then
          pgco_str := '' + IntToStr(page_count) + '  pages'
        else
          pgco_str := 'one  page';

        if banner_paper = True then
          pgco_str := pgco_str + '  of  banner  paper :'
        else
          pgco_str := pgco_str + ' :';
          ...
          if print_entire_pad_flag = True then begin
            if print_group_only_flag = True then
              header_label.Caption :=
                'print group templates only     scaled at : ' + round_str(
                out_factor * 100, 2) + ' %'
            else
              header_label.Caption :=
                'print background templates     scaled at : ' + round_str(
                out_factor * 100, 2) + ' %';
          end
          else
            header_label.Caption :=
              Trim(gauge_str) + '  ' + round_str(scale, 2) + ' mm/ft     scaled at : ' +
          ...
          for sheet_down := 0 to sheet_down_c do begin              // every sheet in row (all 33).
            ...
            with sheet[sheet_down, sheet_across] do begin
              ...
              if print_entire_pad_flag = True then
                bottom_str := ' of  ' + IntToStr(page_count) + '  pages for  ' +
                  box_project_title_str
              else begin
                bottom_str := ' ref :  ' + current_name_str + '    ' +
                  info_form.gauge_label.Caption;
                ...
                if (ABS(controlTemplate.curve.fixedRadius) < max_rad_test) or (controlTemplate.curve.isSpiral) then begin
                  if not controlTemplate.curve.isSpiral then
                    bottom_str := bottom_str + '    curved onto ' +
                      round_str(controlTemplate.curve.fixedRadius, 0) + ' mm radius'
                  else
              ...
              if out_factor <> 1 then
                bottom_str := bottom_str + '    scaled at ' +
                  round_str(out_factor * 100, 2) + ' %';
              ...
              if all_pages = False then begin
                ...
                then begin
                  ...
                  if print_now_form.dont_show_again_checkbox.Checked = False then
                    print_now_form.Show;
                  ...
                  if Application.Terminated = False then
                    Application.ProcessMessages;
                  // otherwise first red outline doesn't show properly.
                  outline_in_red(sheet_down, sheet_across);
                  // show which sheet we mean.
                  ...
                  if (banner_paper = True) and (banner_changed = True)
                  // switch banner infill on-off
                  then begin
                    if printer_printing = True then
                      end_doc(True);
                    goto 123;
                  ...
                  case print_form.ModalResult of
                    ...
                    mrOk: begin
                      print_form.page_panel.Caption :=
                        'preparing  page   ' + temp_str + '  ...';

                      if preview_record_file_made = False
                      // 214b   first page only
                      then begin
                        make_print_preview_screenshot;
                        preview_record_file_made := True;
                      end;
                ...
                if (print_now_bang = False) and (print_form.ModalResult = mrCancel)
                // he's aborted all pages on F12 or Esc
                then begin
                  if printer_printing = True then
                    end_doc(False);
                  EXIT;
              ...
              FillRect(Rect(0, 0, printer_width_indexmax_dots, printer_length_indexmax_dots));
              ...
              if (print_form.page_ident_checkbox.Checked = True) and (page_count > 3)
              // 214a show large page ident if many pages
              then begin
                Brush.Style := bsClear;

                Font.Name := 'Courier New';
                Font.Height := 0 - ABS((page_left_dots - page_right_dots) * 9 div 23);
                // 9/23 trial and error for this font up to z/99
                ...
                if black_white = True then begin
                  ...
                  wm_shift := Round(nom_width_dpi / 30);
                  // watermark outline shift /30 arbitrary
                  Font.Color := $00D0D0D0;               // pale-ish grey
                end;

                ident_left := Round(nom_width_dpi / 10) + page_left_dots +
                  (page_right_dots - page_left_dots - TextWidth(page_num_str)) div 2;
                // nom_width_dpi/10   arbitrary for neatness
                ident_top := page_top_dots + (page_bottom_dots - page_top_dots +
                  Font.Height) div 2;
                ...
                Font.Color := clWhite;
                TextOut(ident_left, ident_top, page_num_str);
                // ink saving, make watermark outline
              ...
              then
                print_shapes_and_sketchboard_items(grid_left, grid_top);    // 206e
              ...
              if printgrid_i = 1 then begin
                ...
                if pad_form.printed_grid_dotted_menu_entry.Checked = True then begin
                  Pen.Style := psDot;
                  ...
                  Pen.Style := psSolid;
                  if impact > 0 then
                    pen_width := 1                   // impact printer or plotter.
                  else
                ...
                if (banner_paper = True) or (print_pages_top_origin <> 0) then
                  now_gridx := 0 - gridx
                else
                  now_gridx := 0;
                //  init grid lines. no need for first line (gets overwritten by trim margins).
                ...
                repeat
                  ...
                  if (now_gridx = 0) and (Pen.Style = psSolid) then
                    Pen.Width := pen_width + 2
                  // thicker datum line (only appears if page origin is negative).
                  else
                    Pen.Width := pen_width;
                  ...
                  TextOut(left_blanking_dots, grid_now_dots -
                    (TextHeight('A') div 2), grid_label_str + ' '); //  add labels.
                ...
                if print_pages_left_origin <> 0 then
                  ...
                  now_gridy := 0;
                //  init grid lines. no need for first line (gets overwritten by trim margin).
                ...
                repeat
                  ...
                  if banner_paper = False then
                    TextOut(grid_now_dots - (TextWidth(grid_label_str) div 2),
                      page_top_dots - (printmargin_wide div 2) - halfmm_dots -
                      TextHeight('A'), grid_label_str) //  add labels.
                  else begin
                    if (sheet_down mod 2) = 0     // alternate pages only for banners.
                    then begin
                      if staggered_pages = True then begin
                        if (sheet_across mod 2) = 0
                        // keep grid labels in line across rows.
                        then
                          bangrid_dots := page_quarter_dots
                        else
                          bangrid_dots := page_3quarter_dots;
                      end
                      else
                        bangrid_dots := page_mid_dots;  // pages not staggered.

                      bangrid_dots := bangrid_dots + TextHeight('A');
                      // to avoid obstructing the alignment marks.

                      TextOut(grid_now_dots -
                        (TextWidth(grid_label_str) div 2), bangrid_dots, grid_label_str);
                    end;
                  end;
                ...
                if banner_paper = False then
                  TextOut(left_blanking_dots, page_top_dots - (printmargin_wide div 2) -
                    halfmm_dots - TextHeight('A'), grid_str)  // add the units string.
                else begin
                  if (sheet_down mod 2) = 0
                  // alternate pages only for banners.
                  then begin
                    if staggered_pages = True then begin
                      if (sheet_across mod 2) = 0
                      // keep grid labels in line across rows.
                    ...
                    bangrid_dots :=
                      bangrid_dots + TextHeight('A');  // to avoid obstructing the alignment marks.

                    TextOut(
                      left_blanking_dots, bangrid_dots, grid_str);
                  end;
                end;
              ...
              then begin
                ...
                if ((print_settings_form.output_centrelines_checkbox.Checked = True) and
                  ...
                  for aq := rdMainRoadCentreLine to rdTurnoutRoadCentreLine do begin
                    if ((plain_track = False) or (aq = rdMainRoadCentreLine)) and
                      (aqyn[aq] = True)
                ...
                if print_settings_form.output_rails_checkbox.Checked = True then begin
                  ...
                  if (rail_infill_i = 0) or ((scale * out_factor) < 0.1)
                  // 0.93.a was 0.75 // less than 2.5% for 4mm scale (control template) (1.43% for 7mm).
                  then begin           //  outline (pen) mode ...
                    //  n.b. this mode does not automatically close the rail-ends.

                    for aq := rdStraightStockGaugeFace to rdAdjTrackMainSideFarOuterFace do
                    begin
                      // 24, 25 centre-lines already done.
                      if (adjacent_edges = False) and (aq in rdAdjacentTracks) then
                        CONTINUE;  // no adjacent tracks in output  // 206b
                      draw_outline_railedge(aq, printcurail_colour);
                    end;//next aq

                    for aq := rdKCrossingCheckMainSideGaugeFace to rdUnused do
                    ...
                    for rail := rdStraightTurnoutWingGaugeFace to rdCurvedStockGaugeFace do
                      draw_fill_rail;  // closure rails and curved stock rail.

                    rail := rdStraightStockGaugeFace;
                    // straight stock rail.
                    draw_fill_rail;

                    for rail := rdMainSideCheckGaugeFace to rdTurnoutSideCheckGaugeFace do
                      draw_fill_rail;  // check rails

                    if adjacent_edges = True    // 206b
                    then begin
                      for rail in [rdAdjTrackTurnoutSideNearGaugeFace,
                          rdAdjTrackTurnoutSideFarGaugeFace,
                          rdAdjTrackMainSideNearGaugeFace, rdAdjTrackMainSideFarGaugeFace] do
                      begin
                        draw_fill_rail;
                      end;
                    end;

                    for rail in [rdKCrossingCheckMainSideGaugeFace,
                        rdKCrossingCheckTurnoutSideGaugeFace] do begin
                      draw_fill_rail;      // K-crossing MS check rails.
                    end;

                    draw_fill_vee;   // now do the vee.

                    // finally draw in or overdraw the planing gauge-faces - (no infill) ...
                    aq := rdStraightTurnoutWingGaugeFace;
                    if (plain_track = False) and (gaunt = False) and
                      (aqyn[aq] = True) and (list_planing_mark_aq1 > 0)
                    {and (drawn_full_aq1=False)}
                    // not if already drawn.
                    then begin
                      move_to.X := get_w_dots(aq, 0);
                      move_to.Y := get_l_dots(aq, 0);
                      for now := 1 to list_planing_mark_aq1{+1} do begin
                        // +1 to overdraw
                        line_to.X := get_w_dots(aq, now);
                        line_to.Y := get_l_dots(aq, now);
                        if check_limits(move_to, line_to) = True then begin
                    ...
                    aq := rdCurvedTurnoutWingGaugeFace;
                    if (plain_track = False) and (gaunt = False) and
                      (aqyn[aq] = True) and (list_planing_mark_aq2 > 0)
                    {and (drawn_full_aq2=False)}
                    // not if already drawn.
                    then begin
                      move_to.X := get_w_dots(aq, 0);
                      move_to.Y := get_l_dots(aq, 0);
                      for now := 1 to list_planing_mark_aq2{+1} do begin
                        // +1 to overdraw
                        line_to.X := get_w_dots(aq, now);
                        line_to.Y := get_l_dots(aq, now);
                        if check_limits(move_to, line_to) = True then begin
              ...
              if (banner_paper = False) and (sheet_down < sheet_co_long) then begin
                if sheet[sheet_down + 1, sheet_across].empty = False
              ...
              if left_blanking_dots > 0 then
                Rectangle(0, 0, left_blanking_dots, printer_length_indexmax_dots + 1);
              if right_blanking_dots <= printer_width_indexmax_dots then
                Rectangle(right_blanking_dots, 0, printer_width_indexmax_dots +
                  1, printer_length_indexmax_dots + 1);

              if (top_blanking_dots > 0) and (banner_paper = False) then
                Rectangle(0, 0, printer_width_indexmax_dots + 1, top_blanking_dots);
              if (bottom_blanking_dots <= printer_length_indexmax_dots) and
                (banner_paper = False) then
                Rectangle(0, bottom_blanking_dots, printer_width_indexmax_dots +
                  1, printer_length_indexmax_dots + 1);
              ...
              if banner_paper = False then
                TextOut(left_blanking_dots, top_blanking_dots, top_str)
              // name and "who for?" string at topleft.
              else begin
                if banner_top_done = False  //sheet_down=0     // only once for banners.
                then begin
                  banner_top_done := True;
                  stry := 0 - Font.Height * 4;
                  //  (Font.Height is negative).
                  TextOut(left_blanking_dots, stry, top_str);
                  // name and "who for?" string at top left.

                  stry := 0 - Font.Height * 7;
                  Font.Assign(print_labels_font);
                  // font for bottom string.
                  Font.Color := calc_intensity(clBlack);
                  // but in black.
                  if print_entire_pad_flag = True then
                    Textout(left_blanking_dots, stry, ' start' + bottom_str)
                  // add the bottom string also.
                  else
                    Textout(left_blanking_dots, stry, ' ' + bottom_str);
                end;
              end;
              ...
              // 214a  for Gordon, see Templot Club message ref: 19595   // background templates
              then begin
                if keep_form.box_file_label.Caption <> '' then
                  last_file_str :=
                    '  printing from: ' + ExtractFileName(keep_form.box_file_label.Caption)
                else
              ...
              if pad_form.show_margin_coordinates_menu_entry.Checked = True then begin
                ...
                this_page_begin_str :=
                  'this page begins: top(X)=' + round_str(grid_top / 100, 2) +
                  'mm, left(Y)=' + round_str(grid_left / 100, 2) + 'mm' + last_file_str;
                // 208g
                this_page_end_str :=
                  'this page ends: bottom(X)=' + round_str(grid_bottom / 100, 2) +
                  'mm, right(Y)=' + round_str(grid_right / 100, 2) + 'mm';
                // 208g
              end
              else begin
                all_pages_origin_str := '';
              ...
              if print_corner_page_numbers_font.Size > 8 then
                Brush.Style := bsClear;

              Textout(page_left_dots + printmargin_wide + 3, page_top_dots + printmargin_wide + 2,
                this_page_begin_str);
              // top left corner
              Textout(page_left_dots + printmargin_wide + 3,
                page_bottom_dots + Font.Height - printmargin_wide - 4,
                page_num_str + '   ' + box_project_title_str + '   ' +
                DateToStr(Date) + ' ' + TimeToStr(Time));
              // bottom left corner
              Textout(page_right_dots - printmargin_wide - TextWidth(
                all_pages_origin_str + page_num_str) - 3, page_top_dots + printmargin_wide + 2,
                all_pages_origin_str + page_num_str);                      // top right corner
              Textout(page_right_dots - printmargin_wide - TextWidth(this_page_end_str) -
                3, page_bottom_dots + Font.Height - printmargin_wide - 4, this_page_end_str);
              // bottom right corner

              if (cal_ok = False) and (print_form.warnings_checkbox.Checked = True)
              //  no calibration done for this printer.
              then begin
                Font.Assign(set_font('Arial', 7, [], printmargin_colour));

                Textout(page_left_dots + printmargin_wide, page_top_dots +
                  printmargin_wide - (Font.Height * 3),
                  '  Warning :  ' + printer_list.Strings[prindex] +
                  '  printer has not been calibrated.  This template may not be dimensionally accurate.  ( press Shift+F5 ).');
              end;

              if (distortions <> 0) and (print_form.warnings_checkbox.Checked = True) then begin
                Font.Assign(set_font('Arial', 7, [], printmargin_colour));

                Textout(page_left_dots + printmargin_wide, page_top_dots +
                  printmargin_wide - (Font.Height * 5),
              ...
              if banner_paper = False then
                Textout(left_blanking_dots, page_bottom_dots +
                  (printmargin_wide div 2) + halfmm_dots,
                  page_str + bottom_str);
              // add the bottom string last.
...
begin
  ...
  if print_entire_pad_flag = True then begin
    if print_group_only_flag = True then
      Caption := '    print  group  templates  only'
    else
      Caption := '    print  background  templates';
  ...
  if black_white = True then
    no_intensity_panel.Visible := True
  else
    no_intensity_panel.Visible := False;
...
procedure print_info(top_str, bottom_str: string);
// print the info page (cut sheets printing).
...
var
  ...
  begin
    with Printer.Canvas do begin
      ...
      if ydots > (printer_length_indexmax_dots - TextHeight(' ') * 2) then begin
...
begin
  if banner_paper = False then
    EXIT;   // this routine only called while printing control template on banner paper is in progress.
  ...
  with Printer.Canvas do begin
    ...
    if get_prindex(prindex) = False then
      EXIT;
    // get current printer index or none available.
    if Tprint_cal(printer_list.Objects[prindex]).cal_data.printer_impact > 0 then
      Font.Style := [];   // not bold on impact printer.

    if box_project_title_str <> '' then
      do_text(Trim(info_form.gauge_label.Caption) + '   for : ' + box_project_title_str)
    else
      do_text(Trim(info_form.gauge_label.Caption));
    ...
    then
      ...
      for line_now := 0 to (info_text_list{info_form.info_memo.Lines}.Count - 1) do
        do_text(info_text_list{info_form.info_memo.Lines}.Strings[line_now]);
    ...
    if Tprint_cal(printer_list.Objects[prindex]).cal_data.printer_impact > 0 then
      Font.Style := [];   // not bold or italic on impact printer.
    ...
    if bottom_str <> '' then begin
      do_text('---------------------------------------------------');
      Font.Assign(print_labels_font);
      Font.Color := clBlack;
      if print_entire_pad_flag = True then
        do_text(' end' + bottom_str)    // add the bottom string last.
      else
        do_text(' ' + bottom_str);
    end;
  end;//with
      ...
      with now_shape do begin
        ...
        then begin
          ...
          if check_limits(move_to, line_to) = True then begin
            case shape_code of
              -1:
                if print_form.omit_pictures_radio.Checked = False    // 206e
                then begin
                  ...
                  if Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgnd_shape.picture_is_metafile = True then begin
                    ...
                    if print_form.picture_stretch_radio.Checked = True then begin
                      try
                        if Tbgshape(
                          bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgimage.image_shape.rotated_bitmap.Empty = True then begin
                          rotate_bitmap(i);
                          Application.ProcessMessages;
                        ...
                        if Tbgshape(
                          bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgimage.image_shape.rotated_bitmap.Monochrome = True then begin
                          Brush.Style := bsSolid;
                    ...
                    if print_form.picture_dots_radio.Checked = True then
                      rotate_bitmap_by_dots(i, p1, p2, grid_left, grid_top,
                        Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgimage.image_shape.image_bitmap.Canvas,
                        Printer.Canvas);

                  end;//bitmap

                  if (print_form.picture_borders_checkbox.Checked = True) or
                    (print_form.picture_outlines_radio.Checked = True) then begin
                    if print_form.picture_outlines_radio.Checked = False then
                      Pen.Width := printpicborder_wide;
                    // picture borders thinner unless an outline.
                    Pen.Color := printshape_colour;
                    Brush.Color := clWhite;
                    Brush.Style := bsClear;
                    Rectangle(move_to.X,
                  ...
                  if print_form.picture_outlines_radio.Checked = True then begin
                    MoveTo(move_to.X, move_to.Y);
                    LineTo(line_to.X, line_to.Y);
                    // printing picture outlines only - draw diagonal line.
          ...
          move_to.X := Round((p1.y * 100 + re_org_y - grid_left) * scaw_out) + page_left_dots;
          // lengthwise arms...
          move_to.Y := Round(((p1.x - arm) * 100 + re_org_x - grid_top) * scal_out) +
            page_top_dots;

          line_to.X := move_to.X;
          line_to.Y := Round(((p1.x + arm) * 100 + re_org_x - grid_top) * scal_out) +
            page_top_dots;
          ...
          move_to.X := Round(((p1.y - arm) * 100 + re_org_y - grid_left) * scaw_out) +
            page_left_dots;
          // widthwise arms...
          move_to.Y := Round((p1.x * 100 + re_org_x - grid_top) * scal_out) + page_top_dots;

          line_to.X := Round(((p1.y + arm) * 100 + re_org_y - grid_left) * scaw_out) +
            page_left_dots;
          line_to.Y := move_to.Y;
          ...
          move_to.X := Round((p1.y * 100 + re_org_y - grid_left) * scaw_out) + page_left_dots;
          move_to.Y := Round(((p1.x - diamond) * 100 + re_org_x - grid_top) * scal_out) +
            page_top_dots;
          ...
          line_to.Y := Round(((p1.x + diamond) * 100 + re_org_x - grid_top) * scal_out) +
            page_top_dots;
...
procedure print_bgnd_marks(grid_left, grid_top: double; maxbg_index: integer;
  rail_joints: boolean);
    ...
    for n := 0 to maxbg_index do begin
      ...
      with now_keep do begin
        ...
        bd := t.boxDims;
          ...
          idnum_str := bd.idNumberStr;  // 208a

          if (bd.usePrintMappingColour) and
            ((mapping_colours_print = 2) or (mapping_colours_print = 3)) and
            (not black_white) and (not grey_shade) then begin
            mapping_colour := calc_intensity(bd.printMappingColour);
            using_mapping_colour := True;
          end;

          if (bd.usePadMarkerColour) and (mapping_colours_print = 4)   // use pad settings instead
            and (not black_white) and (not grey_shade) then begin
            ...
            then begin
              ...
              if (check_limits(infill_points[0], infill_points[1]) = True) and
                ...
                if black_white = True then
                  ...
                  if mapping_colours_print <> -1 then begin
                    if impact > 0 then
                      Brush.Color :=
                        printtimber_colour            // colour plotter, use same as timber outlines.
                    else
                      Brush.Color := printtimber_infill_colour;
                  end
                  else
                ...
                case print_timb_infill_style of
                  ...
                  1:
                    Brush.Style := bsBDiagonal;
                  // hatched. Backward diagonal for the background templates.
                  2:
                    Brush.Style := bsDiagCross;

                  3:
                    if (impact > 0) or (black_white = True) or (mapping_colours_print < 0)
                    then
                      CONTINUE
                    // 209c now no fill   was Brush.Style:=bsBDiagonal    // impact printer or plotter, or printing black and white or in a single colour.
                    else
                      Brush.Style := bsSolid;
            ...
            then begin
              ...
              if check_limit(False, False, move_to) = True then begin
                ...
                then
                  ...
                  if mapping_colours_print < 0 then
                    Font.Color := printbg_single_colour;
                end;
                ...
                if num_str = '' then
                  ...
                  idtb_str := ' ' + idnum_str + '.' + num_str + ' ';
                // 208a  timber number output with template ID
...
var
  ...
  begin
    with Printer.Canvas do begin
      ...
      if (using_mapping_colour = True) and
        ((print_form.black_edges_checkbox.Checked = False) or (rail_edges = False)) then begin
        Pen.Color := mapping_colour;
        EXIT;
      end;

      if (mapping_colours_print < 0) and
        ((print_form.black_edges_checkbox.Checked = False) or (rail_edges = False))
      // 0=normal, 1=rails only, 2=timbers only, 3=rails and timber outlines, 4:=use the PAD colour instead, -1=single colour.
      then begin
        Pen.Color := printbg_single_colour;
        // single colour for all of background templates.
  ...
  function pbg_get_w_dots(q: ERailData; n: integer): integer;
  ...
  function pbg_get_l_dots(q: ERailData; n: integer): integer;
  ...
  procedure pbg_outline_railedge(aq: ERailData; blanking_colour: integer; blank_it: boolean);
  ...
  begin
    ...
    with now_keep do begin
      ...
      if (Length(list_bgnd_rails[aq]) = 0) or (Length(list_bgnd_rails[aq_oppositeFace[aq]]) = 0)
      // data not for both edges?
      then begin
        if Length(list_bgnd_rails[aq]) <> 0 then
          pbg_outline_railedge(aq, 0, False);
        if Length(list_bgnd_rails[aq_oppositeFace[aq]]) <> 0 then
          pbg_outline_railedge(aq_oppositeFace[aq], 0, False);
        EXIT;
      end;
      ...
      with Printer.Canvas do begin
        ...
        if (rail = rdAdjTrackTurnoutSideNearGaugeFace) or
          (rail = rdAdjTrackMainSideNearGaugeFace)   // 0.93.a platforms
        then begin
          if ((using_mapping_colour = True) and (Pen.Color = mapping_colour)) or
            ((mapping_colours_print < 0) and (Pen.Color = printbg_single_colour))
          // 206b
          then
            Brush.Color := Pen.Color
          else
          ...
          case print_platform_infill_style of
            ...
            3:
              if mapping_colours_print < 0          // solid option.
              then
                Brush.Style := bsFDiagonal    // single colour.
              else
                Brush.Style := bsSolid;
          ...
          if ((this_one_trackbed_cess_ts = True) and
            (rail = rdAdjTrackTurnoutSideFarGaugeFace))  // 215a
            or ((this_one_trackbed_cess_ms = True) and (rail = rdAdjTrackMainSideFarGaugeFace))
          // 215a

          then begin
            if ((using_mapping_colour = True) and (Pen.Color = mapping_colour)) or
              ((mapping_colours_print < 0) and (Pen.Color = printbg_single_colour)) then
              Brush.Color := Pen.Color
            else
            ...
            if ((using_mapping_colour = True) and (Pen.Color = mapping_colour)) or
              ((mapping_colours_print < 0) and (Pen.Color = printbg_single_colour)) then
              Brush.Color := calc_intensity(clSilver)  // 214b  - was clGray
            else begin
        ...
        if dots_index > 2 then begin
          ...
          if (fixed_diamond_ends = False) and ((rail = rdStraightTurnoutWingGaugeFace) or
            (rail = rdCurvedTurnoutWingGaugeFace)) then
          ...
          pti := keeps_list[n].boxDims.platformTrackbedInfo;
            ...
            then begin
              ...
              if (rail = rdAdjTrackTurnoutSideNearGaugeFace) and
                (pti.drawTSPlatform) and (pti.drawTSPlatformStartEdge)
              // 0.93.a TS platform start
              then
                pbg_modify_rail_end(0, dots_index, edge_colour, blanking_colour);

              if (rail = rdAdjTrackTurnoutSideNearGaugeFace) and
                (pti.drawTSPlatform) and (not pti.drawTSPlatformEndEdge)
              // 0.93.a TS platform end
              then
  ...
  begin
    with now_keep do begin
      ...
      if (Length(list_bgnd_rails[rdVeePointGaugeFace]) = 0) or
        (Length(list_bgnd_rails[rdVeeSpliceGaugeFace]) = 0) or
        (Length(list_bgnd_rails[rdVeePointOuterFace]) = 0) or
        (Length(list_bgnd_rails[rdVeeSpliceOuterFace]) = 0)
      // not enough data for filled vee.
      then begin
        if Length(list_bgnd_rails[rdVeePointGaugeFace]) <> 0 then
          pbg_outline_railedge(rdVeePointGaugeFace, 0, False);       // draw outline vee...
        if Length(list_bgnd_rails[rdVeeSpliceGaugeFace]) <> 0 then
          pbg_outline_railedge(rdVeeSpliceGaugeFace, 0, False);
        if Length(list_bgnd_rails[rdVeePointOuterFace]) <> 0 then
          pbg_outline_railedge(rdVeePointOuterFace, 0, False);
        if Length(list_bgnd_rails[rdVeeSpliceOuterFace]) <> 0 then
          pbg_outline_railedge(rdVeeSpliceOuterFace, 0, False);
      end
        ...
        aq := rdVeePointGaugeFace;
        edge_started := False;
        for now := 0 to High(list_bgnd_rails[aq]) do begin
        ...
        aq := rdVeePointOuterFace;
        edge_started := False;
        for now := High(list_bgnd_rails[aq]) downto 0 do begin // back along outer-edge.
          x_dots := pbg_get_w_dots(aq, now);
          y_dots := pbg_get_l_dots(aq, now);
          if (w_dims_valid = True) and (l_dims_valid = True) then begin
            edge_started := True;
        ...
        aq := rdVeeSpliceOuterFace;
        edge_started := False;
        for now := 0 to High(list_bgnd_rails[aq]) do begin    // and then turnout side outer edge.
          x_dots := pbg_get_w_dots(aq, now);
          y_dots := pbg_get_l_dots(aq, now);
          if (w_dims_valid = True) and (l_dims_valid = True) then begin
            edge_started := True;
        ...
        aq := rdVeeSpliceGaugeFace;
        edge_started := False;
        for now := High(list_bgnd_rails[aq]) downto 0 do begin
        ...
        with Printer.Canvas do begin
          ...
          if dots_index > 4 then begin
            ...
            pbg_modify_vee_end(
              point_mid_dots_index, point_mid_dots_index + 1, edge_colour, blanking_colour);
            // point rail end.

            pbg_modify_vee_end(
              splice_mid_dots_index, splice_mid_dots_index + 1, edge_colour, blanking_colour);
            // splice rail end.
  ...
  procedure pbg_mark_end(aq1: ERailData; aq1end: integer; aq2: ERailData; aq2end: integer);
  ...
  begin
    pbg_mark_end(rdStraightTurnoutWingGaugeFace, 1, rdStraightTurnoutWingOuterFace, 1);
    // main rail wing rail finish.
    pbg_mark_end(rdCurvedTurnoutWingGaugeFace, 1, rdCurvedTurnoutWingOuterFace, 1);
    // turnout rail wing rail finish.

    pbg_mark_end(rdMainSideCheckGaugeFace, 0, rdMainSideCheckOuterFace, 0);
    // main side check rail start.
    pbg_mark_end(rdMainSideCheckGaugeFace, 1, rdMainSideCheckOuterFace, 1);
    // main side check rail finish.

    pbg_mark_end(rdTurnoutSideCheckGaugeFace, 0, rdTurnoutSideCheckOuterFace, 0);
    // turnout side check rail start.
    pbg_mark_end(rdTurnoutSideCheckGaugeFace, 1, rdTurnoutSideCheckOuterFace, 1);
    // turnout side check rail finish.

    pbg_mark_end(rdVeePointGaugeFace, 0, rdVeeSpliceGaugeFace, 0);    // blunt nose.

    if fixed_diamond_ends = True then begin
      pbg_mark_end(rdStraightTurnoutWingGaugeFace, 0, rdStraightTurnoutWingOuterFace, 0);
      // planed faced of point rails for a fixed-diamond.
      pbg_mark_end(rdCurvedTurnoutWingGaugeFace, 0, rdCurvedTurnoutWingOuterFace, 0);

      pbg_mark_end(rdKCrossingCheckMainSideGaugeFace, 1, rdKCrossingCheckMainSideOuterEdge, 1);
      // MS K-crossing check rails.
      pbg_mark_end(rdKCrossingCheckTurnoutSideGaugeFace, 1,
        rdKCrossingCheckTurnoutSideOuterEdge, 1);     // DS K-crossing check rails.
    end;
  ...
  // 0.93.a draw a complete template in diagrammatic mode
  ...
  begin
    with now_keep do begin
      ...
      if Length(list_bgnd_rails[rdStraightStockGaugeFace]) = 0 then
        EXIT;    // no data for straight stock rail
      if Length(list_bgnd_rails[rdCurvedStockGaugeFace]) = 0 then
        EXIT;    // no data for curved stock rail

      if (Length(list_bgnd_rails[rdVeePointGaugeFace]) = 0) or
        (Length(list_bgnd_rails[rdVeeSpliceGaugeFace]) = 0)
      ...
      now_max := High(list_bgnd_rails[rdStraightStockGaugeFace]);    // straight stock rail
      edge_started := False;

      for now := 0 to now_max do begin
        x_dots := pbg_get_w_dots(rdStraightStockGaugeFace, now);
        y_dots := pbg_get_l_dots(rdStraightStockGaugeFace, now);
        if (w_dims_valid = True) and (l_dims_valid = True) then begin
          edge_started := True;
      ...
      if no_vee = False then begin
        now_max := High(list_bgnd_rails[rdVeePointGaugeFace]);    // point rail
        edge_started := False;

        for now := now_max downto 0 do begin
          x_dots := pbg_get_w_dots(rdVeePointGaugeFace, now);
          y_dots := pbg_get_l_dots(rdVeePointGaugeFace, now);
          if (w_dims_valid = True) and (l_dims_valid = True) then begin
            edge_started := True;
        ...
        now_max := High(list_bgnd_rails[rdVeeSpliceGaugeFace]);    // splice rail
        edge_started := False;

        for now := 0 to now_max do begin
          x_dots := pbg_get_w_dots(rdVeeSpliceGaugeFace, now);
          y_dots := pbg_get_l_dots(rdVeeSpliceGaugeFace, now);
          if (w_dims_valid = True) and (l_dims_valid = True) then begin
            edge_started := True;
      ...
      now_max := High(list_bgnd_rails[rdCurvedStockGaugeFace]);    // curved stock rail
      edge_started := False;

      for now := now_max downto 0 do begin
        x_dots := pbg_get_w_dots(rdCurvedStockGaugeFace, now);
        y_dots := pbg_get_l_dots(rdCurvedStockGaugeFace, now);
        if (w_dims_valid = True) and (l_dims_valid = True) then begin
          edge_started := True;
      ...
      with Printer.Canvas do begin
        ...
        if dots_index > 2 then begin
          ...
          if output_include_boundaries = False then begin
            edge_colour := Pen.Color;  // existing rail edges.
            ...
            if no_vee = False then
              pbg_modify_boundary(ts_mid_dots_index, ts_mid_dots_index + 1,
                edge_colour, blanking_colour);
            // turnout road end.
          end;
        end;
        ...
        then begin
          ...
          t := keeps_list[n];
          if (not t.boxDims.turnoutInfo1.plainTrack)   // not for plain track
            and (not t.turnoutInfo2.semiDiamond)            // not for half-diamond
            and (not t.turnoutInfo2.gaunt)                   // not for gaunt turnout

            and (Length(list_bgnd_rails[rdStraightTurnoutWingGaugeFace]) <>
            0)    // data for straight switch rail
            and (Length(list_bgnd_rails[rdCurvedTurnoutWingGaugeFace]) <> 0)
          // data for curved stock rail
          then begin
            ...
            x_dots := pbg_get_w_dots(rdStraightTurnoutWingGaugeFace, 0);  // ms toe.
            y_dots := pbg_get_l_dots(rdStraightTurnoutWingGaugeFace, 0);

            if (w_dims_valid = True) and (l_dims_valid = True) then begin
              MoveTo(x_dots, y_dots);

              x_dots := pbg_get_w_dots(rdCurvedTurnoutWingGaugeFace, 0);  // ts toe.
              y_dots := pbg_get_l_dots(rdCurvedTurnoutWingGaugeFace, 0);

              if (w_dims_valid = True) and (l_dims_valid = True) then
                LineTo(x_dots, y_dots);
            end;
...
begin          // print background templates...
  ...
  if output_diagram_mode = False then
    print_bgnd_marks(grid_left, grid_top, max_list_index, False);
  // 0.93.a // first print all the background timbering and marks except rail joints.
    ...
    for n := 0 to max_list_index do begin
      ...
      with now_keep do begin
        ...
        ti2 := t.turnoutInfo2;
          ...
          fb_kludge_this := bd.flatbottomKludge;  // 0.94.a
        ...
        if print_settings_form.output_rails_checkbox.Checked = True then begin
          ...
          if ((rail_infill_i = 0) or ((scale * out_factor) < 0.1))
            ...
            for aq := rdStraightStockGaugeFace to rdAdjTrackMainSideFarOuterFace do begin
              // 24, 25 centre-lines already done.
              if (this_one_platforms_trackbed = False) and (aq in rdAdjacentTracks) then
                CONTINUE;  // no adjacent tracks in output  // 206b
              pbg_outline_railedge(aq, 0, False);
            end;
            ...
            then begin
              for rail := rdStraightTurnoutWingGaugeFace to rdCurvedStockGaugeFace do
                pbg_draw_fill_rail;  // closure rails and curved stock rail.
              ...
              for rail := rdMainSideCheckGaugeFace to rdTurnoutSideCheckGaugeFace do
                pbg_draw_fill_rail;  // check rails
            end;
            ...
            then begin
              for rail in [rdAdjTrackTurnoutSideNearGaugeFace,
                  rdAdjTrackTurnoutSideFarGaugeFace, rdAdjTrackMainSideNearGaugeFace,
            ...
            then begin
              ...
              if (Length(list_bgnd_rails[aq]) <> 0) and (gaunt_template = False) and
                (planing_end_aq1 > 0) { and (drawn_full_aq1=False)}    // 0.93.a ex 081
              then begin
              ...
              if (Length(list_bgnd_rails[aq]) <> 0) and (gaunt_template = False) and
                (planing_end_aq2 > 0) { and (drawn_full_aq2=False)} then begin
                move_to.X := pbg_get_w_dots(aq, 0);
                move_to.Y := pbg_get_l_dots(aq, 0);
...
begin
  if (Printer.Printing = True) or (printer_printing = True) then
    index_sheet_wanted := True                          // wanted later.
  else begin
...
begin
  ...
  if printer_printing = False then
    print_preview(True, False, 0);  // redraw in new font.
end;
//__________________________________________________________________________________________
...
begin
  if print_intensity_trackbar.Position < 46 then
    print_intensity_trackbar.Position := print_intensity_trackbar.Position + 5;
  intensity_changed;
end;
//_______________________________________________________________________________________
...
begin
  if print_intensity_trackbar.Position > 4 then
    print_intensity_trackbar.Position := print_intensity_trackbar.Position - 5;
  intensity_changed;
end;
//______________________________________________________________________________
...
begin
  if Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgnd_shape.picture_is_metafile =
    True then
    EXIT;   // 213b
    ...
    with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]) do begin
      ...
      with bgimage.image_shape do begin
        ...
        try
          ...
          alert(2, '    image  preparation  failure',
            'Preparation of the following picture shape for printing has failed:' +
            '||' + str + '||The failure was caused by insufficient resources in your system.'
        ...
        if inheight < 32767 then
          print_form.row_progressbar.Max := inheight    // Delphi 16-bit limits..
        else
          print_form.row_progressbar.Max := 32767;
        ...
        for inrow := 0 to (inheight - 1) do begin
          for incol := 0 to (inwidth - 1) do begin
            ...
            if pixel_colour <> clWhite then
              rotated_bitmap.Canvas.Pixels[inheight - 1 - inrow, incol] :=
                calc_intensity(pixel_colour);
          end;//for
          print_form.row_progressbar.StepIt;
        end;//for
...
begin
  ...
  try
    ...
    if (width_pixels < 1) or (height_pixels < 1) then
      EXIT;
    ...
    if height_pixels < 32767 then
      print_form.row_progressbar.Max := height_pixels    // Delphi 16-bit limits..
    else
      print_form.row_progressbar.Max := 32767;
    print_form.row_progressbar.Position := 0;
    ...
    for inrow := 0 to (height_pixels - 1) do begin
      for incol := 0 to (width_pixels - 1) do begin
        ...
        xmm := p1.x + incol * width_mm_per_pixel;
        // picture co-ordinates in mm..
        ymm := p1.y + (height_pixels - 1 - inrow) * height_mm_per_pixel;
        // (top-left corner of pixel).

        pixel_X := Round((ymm * 100 + re_org_y - grid_left) * scaw_out) + page_left_dots;
        // printer canvas co-ordinates..
        ...
        if check_if_on_page(out1, out2, True) = False then
          CONTINUE;  // blob not on this page or outside limits.

        print_form.Caption := '  ' + name_str + '  row: ' + IntToStr(inrow) +
          '  col: ' + IntToStr(incol);


        pixel_colour := calc_intensity(source_canv.Pixels[incol, inrow]);
        if pixel_colour = clWhite then
          CONTINUE;  // no need to print white! (make transparent).
...
begin
  repeat
    i := alert(3, '    print  picture  shapes  ( dots  method )',
      ...
      '', 'more  information', 'omit  pictures',
      'O K  -  print  pictures  using  dots  method', 'print  pictures  as  outlines  only',
      'print  pictures  using  normal  method  instead', 2);
    case i of
      2:
        if help(0, picture_help_str, 'more  about  picture  shapes') = 1 then
          bgnd_form.picture_shapes_help_menu_entry.Click;
      // !!! don't use alert_help, print_form won't permit being deactivated.
      3:
        omit_pictures_radio.Checked := True;
      5:
        picture_outlines_radio.Checked := True;
      6:
        picture_stretch_radio.Checked := True;
    end;//case
  until i <> 2;
end;
...
begin
  ...
  if (corner1.X < 0) and (corner2.X < 0) then
    EXIT;  // not on this page.
  if (corner1.X > printer_width_indexmax_dots) and (corner2.X > printer_width_indexmax_dots) then
    EXIT;  // not on this page.

  if (corner1.Y < 0) and (corner2.Y < 0) then
    EXIT;  // not on this page.
  if (corner1.Y > printer_length_indexmax_dots) and (corner2.Y > printer_length_indexmax_dots) then
    EXIT;  // not on this page.

  if check_within_limits = True then begin
    if check_limits(corner1, corner2) = False then
      EXIT;
  end;
  Result := True;
end;
...
begin
  if help(0, print_help_str, 'about  mapping  colours') = 1 then
...
begin
  if print_sketchboard_items_checkbox.Checked = False then
    EXIT;
