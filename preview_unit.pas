...
function page_info(pad, no_alerts, non_print_output: boolean; output_code: integer): boolean;
// get current printer info and calc page limits etc.
function print_preview(no_alerts, non_print_output: boolean; output_code: integer): boolean;
// get current printer info and calc page limits etc.

function get_fit_all_templates_size_mm(include_control, include_bgnd, group_only: boolean): Tpex;
// extracted 0.93.a
function get_fit_all_shapes_size_mm(showing_trackpad_only, showing_output_only: boolean): Textents;
// 0.93.a

procedure print_index_sheet(pdf: boolean);
...
begin
  ...
  for n := 0 to (bgnd_form.bgnd_shapes_listbox.Items.Count - 1) do begin
    ...
    with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[n]).bgnd_shape do
    begin  // next shape.

      //  0=line, 1=rectangle, 2=circle, 3=text, 4=target mark, -1=picture (bitmap image).

      if (showing_trackpad_only = True) and ((hide_bits and $01) <> 0) then
        CONTINUE;   // 214a  byte,  0=normal,  1=hide on trackpad,  2=hide on output,  3=hide both
      if (showing_output_only = True) and ((hide_bits and $02) <> 0) then
        CONTINUE;
...
begin
  ...
  if (include_control = True) and (output_diagram_mode = False)
  // control template not included in diagram mode
  then begin

    yd := y_datum * 100;    // (y_datum is in mm from left edge of sheet).

    if xy_max[0] > max_long then
      max_long := xy_max[0];
    if (yd + xy_max[1]) > max_wide then
      max_wide := yd + xy_max[1];
  end;

  // then add background templates...

  if (include_bgnd = True) and (keeps_list.Count > 0) then begin
...
function page_info(pad, no_alerts,{pdf}non_print_output: boolean; output_code: integer): boolean;
  // get current printer info and calc page limits etc.
...
begin
  ...
  then begin
    ...
    if (output_code < 0) or (output_code > 4) then begin
    ...
    try
      ...
      printer_width_dots := Printer.PageWidth;
      // printer page-width in dots.    // OT-FIRST
      printer_length_dots := Printer.PageHeight;
      // printer page-length in dots.   // OT-FIRST
    ...
    if (valid_printer_data = False) or (nom_width_dpi < 1) or (nom_length_dpi < 1) or
      (printer_width_dots < 1) or (printer_length_dots < 1)   // division by zero, or negative.
    then begin
      if pad = False then
    ...
    case Printer.Orientation of
      ...
      poPortrait: begin
        cal_wide_dpi := nom_width_dpi / head_factor;
        // adjust to actual dpi...
  ...
  if (non_print_output = True) and (output_code = 0) then
    per_printer_margins := True;  // added 205e PDF  use default trim margins.

  if (Round(print_width) <> previous_print_width) or (Round(print_length) <>
    previous_print_length) then
    per_printer_margins := True;    // use default trim margins.

  previous_print_width := Round(print_width);
  // for check next time (nearest 1/100th mm) ...
  previous_print_length := Round(print_length);

  printer_width_indexmax_dots := printer_width_dots - 1;
  // printer width max index in dots -1 (index 0-).
  ...
  if non_print_output = True then begin
    case output_code of
      0: begin                          // PDF
        ...
        page_margin_bottom_mm := 4.5 + print_length / 100 - pdf_height_mm;
        // bottom margin 4.5mm, adjust for any rounding
        page_margin_right_mm := 2.0 + print_width / 100 - pdf_width_mm;
        // right margin 2mm, adjust for any rounding

        if page_margin_bottom_mm < 1.0 then
          page_margin_bottom_mm := 1.0;
        if page_margin_right_mm < 1.0 then
          page_margin_right_mm := 1.0;    // 1mm arbitrary.

        left_blanking_mm := 0;                       // no left blanking.
        top_blanking_mm := 0;                        // no top blanking.
        right_blanking_mm := print_width / 100 + 5.0;
        // no right blanking (+5 mm arbitrary).
      ...
      3: begin                       // create image file
        ...
        // 0.93.a  28-7-2010  no trim margins on created image files
        page_margin_bottom_mm := 0;
        page_margin_left_mm := 0;
        page_margin_right_mm := 0;
        left_blanking_mm := 0;
        top_blanking_mm := 0;
        right_blanking_mm := (create_image_height_dots / create_image_dpi) * 25.4 + 5.0;
        // no right blanking (+5 mm arbitrary).
        bottom_blanking_mm := (create_image_width_dots / create_image_dpi) * 25.4 + 5.0;
        // no bottom blanking (+5 mm arbitrary).
      end;
    ...
    then begin
      ...
      // 6 mm top trim margin.      mod 0.76.a was 6.5 mm
      page_margin_bottom_mm := 4.5;
      // 4.5 mm bottom trim margin. mod 0.76.a was 4 mm
      page_margin_left_mm := 7.0;    // 7 mm left trim margin.
      ...
      if left_blanking_mm < 0 then
        left_blanking_mm := 0;

      right_blanking_mm := print_width / 100 - page_margin_right_mm + 2.0;
      // adjust right blanking (2mm arbitrary).
      if right_blanking_mm < 0 then
        right_blanking_mm := 0;

      top_blanking_mm := page_margin_top_mm - 6.0;         // adjust top blanking.
      if top_blanking_mm < 0 then
        top_blanking_mm := 0;

      bottom_blanking_mm := print_length / 100 - page_margin_bottom_mm + 4.5;
      // adjust right blanking (4.5mm arbitrary).
      if bottom_blanking_mm < 0 then
        bottom_blanking_mm := 0;
    end;
  end;
  ...
  page_width := print_width - (page_margin_left_mm + page_margin_right_mm) * 100;
  // width per page between margins in 1/100th mm.
  page_length := print_length - (page_margin_top_mm + page_margin_bottom_mm) * 100;
  // length ditto.
  ...
  if out_factor < minfp then
    out_factor := 1;       // can't go that small!

  if (non_print_output = False) and (banner_paper = True) then
    banner_top_offset := 2500   // banners start 25 mm from top of first page.
  else
    banner_top_offset := 0;     // single sheets all start at the top.
  ...
  then begin
    try
      ...
      out_single_factor_long := (page_length - banner_top_offset) / (max_long - page_top_offset);
      // banner first page has 25 mm top margin.
    except
      out_single_factor_wide := out_factor / 10;   // arbitrary size if calcs fail.
      out_single_factor_long := out_factor / 10;
    end;//try

    if out_single_factor_wide < out_single_factor_long
    // set maximum factor to fit both ways...
    then
      out_factor := out_single_factor_wide * 0.98     // allow 2% right margin space
    else
      out_factor := out_single_factor_long * 0.98;

    if out_factor > 1.0 then
      out_factor := 1.0;             // but don't enlarge it to fit page,
  ...
  page_top_offset := page_top_offset - banner_top_offset / out_factor;
  // 1/100th mm.  (banner offset needs to be actual, so the divide here cancels subsequent scaling).
  ...
  scal := cal_long_dpi / 2540;
  //  length scaling factor in dots per 1/100th mm. Used for trim margins, etc.
  scaw := cal_wide_dpi / 2540;       //  ditto width scaling factor (+ve for the printer).

  scaw_out := scaw * out_factor;
  // dots per 1/100th mm. (at required output scaling). Used for grid and template.
  ...
  page_mid_dots := Round((page_margin_top_mm * 100 + (page_length / 2)) * scal);
  // for trim align marks ...
  ...
  sheet_co_wide := Trunc((max_wide - page_left_offset) / page_width_out);
  // max sheet index (no of sheets -1) across (calc from left page margin).
  sheet_co_long := Trunc((max_long - page_top_offset) / page_length_out);

  if (sheet_co_wide > 0) and (staggered_pages = True) then
    sheet_co_long := sheet_co_long + 1;   // Add 1 sheet to allow for staggered joins.

  if sheet_co_wide > sheet_across_c then begin
    sheet_co_wide := sheet_across_c;
    if no_alerts = False then
      if crop_alert('width') = True then
        EXIT;
  end;

  if sheet_co_long > sheet_down_c then begin
    sheet_co_long := sheet_down_c;
    want_bottom_margin := True;
    if no_alerts = False then
      if crop_alert('length') = True then
        EXIT;
  end
  else
    want_bottom_margin := False;

  for sheet_down := 0 to sheet_down_c do begin
    // every sheet (all 33*26 = 858 sheets).
  ...
  for sheet_across := 0 to sheet_co_wide do begin
    for sheet_down := 0 to sheet_co_long do begin            //  only the sheets we need.
      ...
      with sheet[sheet_down, sheet_across] do begin
        ...
        grid_top := (sheet_down * page_length_out) + page_top_offset;
        // data offset to the top of this page (1/100th mm) (25 mm start offset for banner printing).

        if (sheet_across mod 2 <> 0) and (staggered_pages = True) then
          grid_top := grid_top - page_length_out / 2;   // stagger odd columns of sheets.
            ...
            then begin
              for now := 0 to nlmax_array[aq] do begin
                ...
                if (across > grid_left) and (across < grid_right) and
                  ...
                  empty := False;
                  //  something on this sheet in the control template.
                  any_sheets := True;
                  //  there is a template to print.
                end;
              end;
            end;
          end;//for next aq
        ...
        { and (pad_form.print_keeps_on_menu_entry.Checked=True) } then begin
              ...
              for aq in ERailData do begin
                ...
                for now := 0 to array_max do begin
                  ...
                  if (across > grid_left) and (across < grid_right) and
                    (down > grid_top) and (down < grid_bottom) then begin
                    empty := False;
                    //  something on this sheet in the background.
  ...
  if any_sheets = False then begin
    if no_alerts = True then
      EXIT
    else begin
      ...
      then begin
        alert(6, '  no  data  for  print  control  template  preview',
          ...
          + ' Or moved it beyond the printing area of ' + IntToStr(sheet_down_c + 1) +
          ' pages long by ' + IntToStr(sheet_across_c + 1) + ' pages wide?' +
        ...
        alert(6, '  no  data  for  print  preview',
          ...
          '||Have you cleared or wiped all background templates while the control template is being omitted or is completely below or to the left of the page origin?' + '||Or moved the entire drawing beyond the printing area of ' + IntToStr(sheet_down_c + 1) + ' pages long by ' + IntToStr(sheet_across_c + 1) + ' pages wide?' + '||Or switched off the rails and the track centre-lines on the generator for the control template and all of the background templates?',
          '', '', '', '', 'cancel  printing', '', 0);
        EXIT;
      end;
    end;
  ...
  if (non_print_output = False) and (banner_paper = True) and
    (print_form.banner_fill_checkbox.Checked = True) then begin
    for sheet_across := 0 to sheet_co_wide do begin
      // check for any column infills needed...

      column_started := False;                         // not up to first non-empty page yet.

      for sheet_down := 0 to sheet_co_long do begin

        if sheet[sheet_down, sheet_across].empty = False then
          column_started := True
        else begin
          if (column_started = True) and (sheet_down < sheet_co_long)
          // blank page and the column has previously started and there are more pages in the column...
          then begin
            for next_page_down := (sheet_down + 1) to sheet_co_long do begin
              if sheet[next_page_down, sheet_across].empty = False then
                sheet[sheet_down, sheet_across].empty := False;  // we need to infill this one.
            end;
          end;
        end;
      end;//for sheet_down
...
function print_preview(no_alerts,{pdf}non_print_output: boolean; output_code: integer): boolean;
  // get current printer info and calc page limits etc.
  ...
  begin
    ...
    if (endmarks_yn[aq1, aq1end] = True) and (endmarks_yn[aq2, aq2end] = True) then begin
      ...
      with prev_canvas do begin
        ...
        move_to.X := Round(p1.X * pvsx) + left_dots;
        move_to.Y := bottom_dots - Round((p1.Y + ypd) * pvsy);
        line_to.X := Round(p2.X * pvsx) + left_dots;
        line_to.Y := bottom_dots - Round((p2.Y + ypd) * pvsy);
...
begin
  ...
  try
    ...
    with prev_canvas do begin
      ...
      pvsx := (xmax - 10) / ((sheet_co_long + 1) * page_length_out);
      // x screen pixels per 1/100th mm (scaled), to fit all pages.
      pvsy := (ymax - 10) / ((sheet_co_wide + 1) * page_width_out);   // y ditto.

      if pvsx > pvsy then
        pvsx := pvsy;            // make sure scales are equal.
      if pvsy > pvsx then
        pvsy := pvsx;
      // set left origin...
      ...
      if staggered_pages = True then begin
        for n := 0 to sheet_co_wide do begin
          // search for any non-empty staggered column first pages.
          if (n mod 2) = 0 then
            CONTINUE;     // not even columns.
          if sheet[0, n].empty = False then begin
            left_dots := left_dots + Round((page_length_out * pvsx) / 2);
            // offset to include all staggered sheets.
            BREAK;
          end;
        end;//for
      end;
      ...
      if bgnd_form.output_shapes_in_front_of_sb_checkbox.Checked = True then
        preview_sketchboard_items(prev_canvas);  // 206e
      ...
      if (pad_form.preview_background_shapes_menu_entry.Checked = True) // 206e
        ...
        font_size := limits(1, Font.Size * 2, font_size, dummy_i);
        // limit font size on screen.
              ...
              then begin
                ...
                if check_limits(move_to, line_to) = True then begin
                  case shape_code of
                    -1: begin     // picture = bitmap image.
                      ...
                      try
                        ...
                        if print_form.picture_borders_checkbox.Checked =
                          True then begin
                          Brush.Color := paper_colour;
                          Brush.Style := bsClear;
                          Rectangle(
                  ...
                  if shape_code = 3 then
                    TextOut(move_to.X + 1, move_to.Y + 1, ' ' + shape_name);
                  // insert label text in rectangle box.
                end;
              end
      ...
      if (print_entire_pad_flag = True) and (keeps_list.Count > 0) then begin
          ...
          pti := bd.platformTrackbedInfo;
              ...
              with now_keep do begin
                ...
                for aq in ERailData do begin
                  ...
                  if (not t.drawCentrelineOnly) and (something_drawn) and
      ...
      while (TextWidth(' page  w/99  ') > page_length_pixels) and (Font.Size > 12)
        // do we need to change font size (to minimum 12 pt)?
        do
        Font.Size := Font.Size - 1;

      if TextWidth(' page  w/99  ') < page_length_pixels
      // now room between margin lines for full string?
      then
        page_prefix_str := ' page  '
      else begin                         // shorten label and try full-size font again.
        page_prefix_str := ' ';
        Font.Size := Round(font_size);
      end;

      while (TextWidth(page_prefix_str + 'w/99  ') > page_length_pixels) and
        (Font.Size > 6)   // still need to change font size (minimum 6 pt)?
        do
        Font.Size := Font.Size - 1;
          ...
          with sheet[sheet_down, sheet_across] do begin
            ...
            then begin
              ...
              then begin                           // yes, mark the corner dotted.
                ...
                move_to.X := Round(grid_top * pvsx) + left_dots;
                move_to.Y := bottom_dots - Round(grid_left * pvsy);  // paper top left corner.
                line_to.X := move_to.X;
                line_to.Y := bottom_dots - Round(grid_right * pvsy);
                // paper top margin.
                if check_limits(move_to, line_to) = True then begin
                  MoveTo(move_to.X, move_to.Y);
                  LineTo(line_to.X, line_to.Y);
                ...
                line_to.X := Round(grid_bottom * pvsx) + left_dots;
                line_to.Y := move_to.Y;                       // paper left margin.
                if check_limits(move_to, line_to) = True then begin
                  MoveTo(move_to.X, move_to.Y);
                  LineTo(line_to.X, line_to.Y);
                end;
                ...
                if text_X < 0 then
                  text_X := 0;              // (in case of first banner page).

                page_str :=
                  ' (' + page_prefix_str + Chr(sheet_across + 97) + '/' +
                  IntToStr(sheet_down + 1) + ' ) ';

                TextOut(text_X, bottom_dots - Round(grid_left * pvsy) +
                  (Font.Height * 3 div 2), page_str);
              end;
            ...
            if banner_paper = True then
              Pen.Style := psDot       // for vertical lines on screen
            else
              Pen.Style := psSolid;
            Pen.Width := 1;
            ...
            move_to.X := Round(grid_top * pvsx) + left_dots;
            move_to.Y := bottom_dots - Round(grid_left * pvsy);         // paper top left.

            line_to.X := Round(grid_top * pvsx) + left_dots;
            line_to.Y := bottom_dots - Round(grid_right * pvsy);        // paper top margin.
            if check_limits(move_to, line_to) = True then begin
              MoveTo(move_to.X, move_to.Y);
              LineTo(line_to.X, line_to.Y);
            end;
            ...
            move_to := line_to;
            line_to.X := Round(grid_bottom * pvsx) + left_dots;
            line_to.Y := bottom_dots - Round(grid_right * pvsy);        // paper right margin.
            if check_limits(move_to, line_to) = True then begin
              MoveTo(move_to.X, move_to.Y);
              LineTo(line_to.X, line_to.Y);
            end;
            ...
            move_to := line_to;
            line_to.X := Round(grid_bottom * pvsx) + left_dots;
            line_to.Y := bottom_dots - Round(grid_left * pvsy);   // paper bottom margin.
            if check_limits(move_to, line_to) = True then begin
              MoveTo(move_to.X, move_to.Y);
              LineTo(line_to.X, line_to.Y);
            end;
            ...
            move_to := line_to;
            line_to.X := Round(grid_top * pvsx) + left_dots;
            line_to.Y := bottom_dots - Round(grid_left * pvsy);      // paper left margin.
            if check_limits(move_to, line_to) = True then begin
              MoveTo(move_to.X, move_to.Y);
              LineTo(line_to.X, line_to.Y);
            end;
            ...
            page_str := page_prefix_str + Chr(sheet_across + 97) + '/' +
              IntToStr(sheet_down + 1) + ' ';
...
const
  multi_print_str: string = '     Print / PDF  run  overlap.' +
    ...
    + '||To move the page origin with the mouse select the ACTION > MOUSE ACTIONS:PAD > MOVE PAGE ORIGIN menu item (SHIFT+CTRL-F10), or to enter a new page origin directly' + ' select the OUTPUT > PAGE ORIENTATION / ORIGIN > SET PAGE ORIGIN... menu item.' + '||If the drawing cannot be fitted within the print run by moving the page origin, it will be necessary to print it in multiple runs.' + ' After printing the first run of pages, select one of the OUTPUT > PAGE ORIENTATION / ORIGIN > MULTIPLE PRINT RUNS menu items to prepare for the next run and to preserve the page joins between runs.' + '||N.B. Do not change the print size between multiple print runs.' + '||After printing multiple runs the page origin can be restored to the drawing datum by selecting the OUTPUT > PAGE ORIENTATION / ORIGIN > RESET PAGE ORIGIN menu item.';
...
begin
  ...
  repeat
    i := alert(2, '    overlap  for  PDF  or  printing  the  ' + size_str,
      'This drawing overlaps the maximum print run size of ' + IntToStr(
      sheet_down_c + 1) + ' pages in length and/or ' + IntToStr(sheet_across_c + 1) +
      ' pages in width.' + '||At your current paper size, this represents a maximum printed area of '
      + round_str((sheet_down_c + 1) * page_length / 100, 2) + ' mm long and ' +
      round_str((sheet_across_c + 1) * page_width / 100, 2) + ' mm wide.' +
      '||Unless you move the page origin it will have to be printed in multiple runs, or at a smaller size, or on larger sheets of paper.' + '||The current print size is ' + round_str(out_factor * 100, 2) + ' %', '', '', '', '?  help', 'cancel  print / PDF    ', 'O K    ', 4);
    case i of
      4:
        alert_help(0, multi_print_str, '');
      5:
        Result := True;
    end;//case
  until i <> 4;
...
begin
  if continue_button.Showing = True then
    continue_button.SetFocus;    // don't want focus on this button next time.
...
begin
  ...
  try
    ...
    with Printer.Canvas do begin
      ...
      top_str := ' ' + DateToStr(Date) + '  ' + TimeToStr(Time) + '      TEMPLOT V:' +
        GetVersionString(voShort) + '  from  Martin  Wynne   templot.com';
      TextOut(0, 0, top_str);

      Font.Assign(set_font('Comic Sans MS', 8, [], clBlack));
      top_str := ' page map for :    ' + print_form.header_label.Caption +
        '     ' + print_form.origin_label.Caption;
      if box_project_title_str <> '' then
        top_str := top_str + '          for :  ' + box_project_title_str;
      TextOut(0, TextHeight(' '), top_str);


      xmax := printer_width_indexmax_dots - 20;
      // 20 arbitrary margin   (1/15" at 300dpi)
      ymax := printer_length_indexmax_dots - 20;

      // calc preview scaling to show all the pages...

      pvsx := (xmax - 20) / ((sheet_co_long + 1) * page_length_out);
      // x screen pixels per 1/100th mm (scaled), to fit all pages.
      pvsy := (ymax - 20 - TextHeight(' ') * 3) / ((sheet_co_wide + 1) * page_width_out);
      // y ditto.
      ...
      begin
        ...
        Font.Size := Round(font_size);
        // font scaled down to the pad contents (but not up more than double).
              ...
              if check_limits(move_to, line_to) = True then begin
                case shape_code of
                  -1: begin                            // picture.
                    ...
                    Rectangle(move_to.X, move_to.Y, line_to.X, line_to.Y);
                    MoveTo(move_to.X, move_to.Y);
                    LineTo(line_to.X, line_to.Y);  // picture outlines only - draw diagonal line.
      ...
      // use the printer font for the page numbers.
      page_length_pixels := Round(page_length_out * pvsx);      //  used for page label check.

      while (TextWidth(' w/9  ') > page_length_pixels) and (Font.Size > 6)
        // need to change font size (minimum 6 pt)?
        do
        Font.Size := Font.Size - 1;
          ...
          with sheet[sheet_down, sheet_across] do begin
            ...
            then begin
              ...
              then begin                           // yes, mark the corner dotted.
                ...
                page_str :=
                  ' ( ' + Chr(sheet_across + 97) + '/' + IntToStr(sheet_down + 1) + ' ) ';

                TextOut(text_X, bottom_dots - Round(grid_left * pvsy) +
                  (Font.Height * 2), page_str);
              end;
              CONTINUE;    // draw pages only if not empty.
            end;
