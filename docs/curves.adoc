...
determines which segment to call to calculate the curve information at the requested point. `distance` is the distance along the curve, and `offset` is the perpendicular distance from the curve, where a positive value is to the right of the curve. 

Between them, `distance` and `offset` effectively define a 2-D coordinate system relative to the curve path.

The outputs of the CalculateCurveAt method are the point at the specified distance/offset along the curve, the direction of the curve at the point, 
...
TCurve provides another public method to calculate the equivalent distance along a curve for a given distance at an offset from the curve.
[source,pascal]
....
...
This function will calculate the distance `d1` along the curve, such that the point (d1,offset) is equal to the point (d,offset) moved along the offset curve by the amount `distanceFromOffset`.
