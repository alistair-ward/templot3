...
=== type
The Pascal type for the property. `Integer`, `Double`, `String` and `TDateTime` are supported directly. Other types can be specified if appropriate supporting methods (for conversion to/from string) are provided.
...
=== extraSetCode
Specifies additional Pascal code that will be inserted to the Set method for the property. This can be a single line or a multi-line block literal.
...
Enumerated types require additional supporting functions for Saving/Restoring objects to YAML. These support functions are generated automatically if the enumerated type is defined in a YAML block.

Each enumerated type should be defined in a separate YAML document. YAML documents can be defined in their own comment block, or multiple documents can be defined with a single comment block.

An enumerated type is defined by its type name, and the values it can take:
...
The `---` and `...` are the YAML document delimiters. Multiple enumerated types can be defined in a single block (along with the class definition as well).
...
The enumerated types will be declared in the interface section before the class definition. The supporting `StrTo...` and `...ToStr` functions will be declared at the bottom of the interface section, and then defined in the implementation section.
